...
export function orderRepository(schemaName: string) {
  ...
  const orders = schema.table('orders', {
    id: text().primaryKey(), tenant_id: text().notNull(), customer_id: text(),
    status: text().notNull(), priority: integer().notNull(), note: text(),
  });
  const items = schema.table('order_items', {
    id: text().primaryKey(), tenant_id: text().notNull(), order_id: text(), amount: integer().notNull(),
  });
  return {
    orders, items,
    ...
    async total(tx: DrizzleCommandDb, id: string) {
      const rows = await tx.select({amount: items.amount}).from(orders).innerJoin(items, eq(orders.id, items.order_id)).where(eq(orders.id, id));
