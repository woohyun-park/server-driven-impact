...
class BoundPool extends pg.Pool {
  ...
  connect() {
    throw new Error('BOUND_POOL_CANNOT_RESERVE');
  }
}
function factory(db) {
  return {
    provider: 'postgres',
    adapterName: 'sdi-prisma-feasibility',
    async connect() {
      ...
      return {
        provider: 'postgres',
        adapterName: 'sdi-prisma-feasibility',
        queryRaw: q => inner.queryRaw(q),
        executeRaw: q => inner.executeRaw(q),
        executeScript: () => {
        ...
        async startTransaction() {
          ...
          return {
            ...
            queryRaw: q => adapter.queryRaw(q),
            executeRaw: q => adapter.executeRaw(q),
            async commit() {
              resolveGate();
              await completion;
              evidence.push('prisma-savepoint-commit');
            },
            async rollback() {
              rejectGate(marker);
              try {
              ...
              evidence.push('prisma-savepoint-rollback');
            },
          };
        },
      };
...
async function command(token, fn, failDrain = false) {
  ...
  try {
    await client.query(
      'CREATE TEMP TABLE IF NOT EXISTS sdi_prisma_collector(token text, kind text, id int) ON COMMIT PRESERVE ROWS',
    );
    ...
    record.beforeCommit = (
      await client.query('SELECT kind,id FROM sdi_prisma_collector WHERE token=$1 ORDER BY kind,id', [token])
    ).rows;
    db.close();
    ...
    record.facts = (
      await client.query('DELETE FROM sdi_prisma_collector WHERE token=$1 RETURNING kind,id', [token])
    ).rows;
...
try {
  ...
  const success = await command('success', async prisma => {
    ...
    return prisma.effect.create({
      data: { id: 1, value: 'direct', children: { create: { id: 101 } } },
      include: { children: true },
    });
  });
  assert.equal(success.result.children[0].id, 101);
  assert.equal(
    success.record.beforeCommit.some(f => f.kind === 'sdi_prisma_audit'),
    false,
  );
  assert.deepEqual(success.record.facts.map(f => `${f.kind}:${f.id}`).sort(), [
    'sdi_prisma_audit:1',
    'sdi_prisma_child:101',
    'sdi_prisma_effect:1',
  ]);
  ...
  await command('savepoint', async prisma => {
    await assert.rejects(() =>
      prisma.effect.create({ data: { id: 2, value: 'cancelled', children: { create: { id: 101 } } } }),
    );
    await prisma.effect.create({ data: { id: 3, value: 'retained' } });
  }).then(({ record }) =>
    assert.deepEqual(record.facts.map(f => `${f.kind}:${f.id}`).sort(), ['sdi_prisma_audit:3', 'sdi_prisma_effect:3']),
  );
  await assert.rejects(
    () =>
      command('rollback', async prisma => {
        await prisma.effect.create({ data: { id: 4, value: 'rollback' } });
        throw new Error('ROLLBACK_REQUEST');
      }),
    /ROLLBACK_REQUEST/,
  );
  assert.equal(
    (await pool.query('SELECT count(*)::int AS count FROM sdi_prisma_effect WHERE id IN (2,4)')).rows[0].count,
    0,
  );
  await assert.rejects(
    () =>
      command('commit-failure', async prisma => {
        await prisma.$executeRawUnsafe('INSERT INTO sdi_prisma_audit VALUES (999)');
      }),
    /foreign key/,
  );
  await assert.rejects(
    () =>
      command(
        'drain-failure',
        async prisma => {
          await prisma.effect.create({ data: { id: 5, value: 'committed' } });
        },
        true,
      ),
  ...
  const concurrent = await Promise.all(
    [6, 7].map(id => command(`parallel-${id}`, p => p.effect.create({ data: { id, value: 'parallel' } }))),
  );
    ...
    JSON.stringify(
      {
        versions: {
          ...
          postgres: (await pool.query('SHOW server_version')).rows[0].server_version,
        },
        checks: 'PASS',
        evidence,
      },
