...
try {
  ...
  for (const mode of ['native', 'broad', 'narrow']) {
    await admin.unsafe(
      `create table "${schema}".${mode}(id integer primary key,value integer not null);grant select,insert,update,delete on "${schema}".${mode} to routine_runtime`,
    );
    ...
    const queries = {
      list: {
        ...
        plan: q.select('rows', mode === 'narrow' ? { where: [q.eq('id', q.input('id'))] } : {}),
      },
    };
    await admin.unsafe(
  ...
  for (const rows of [1, 1000, 10000])
    for (const mode of ['native', 'broad', 'narrow']) {
      await admin.unsafe(
        `truncate "${schema}".${mode};insert into "${schema}".${mode} select n,0 from generate_series(1,${rows}) n`,
      );
      const durations = [];
      let bytes = 0;
      for (let sample = -2; sample < samples; sample++) {
        const start = performance.now();
        const statement = `update "${schema}".${mode} set value=value+1`;
        const response =
          mode === 'native'
            ? await database.begin('isolation level repeatable read', tx => tx.unsafe(statement))
            : await engines[mode].command({ scope: 'bench' }, db => db.execute(new Sql(statement)));
        if (sample >= 0) {
      ...
      const percentile = p => Number(durations[Math.ceil(p * durations.length) - 1].toFixed(3));
      results.push({
        rows,
        mode,
        samples,
  ...
  const report = {
    ...
    validation: 'explicit-not-run-in-samples',
    samples,
    businessSelects,
    results,
  };
