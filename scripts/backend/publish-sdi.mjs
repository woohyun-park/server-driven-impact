...
if (!['dry-run', 'next', 'latest'].includes(channel))
  throw new Error('Usage: node scripts/backend/publish-sdi.mjs <dry-run|next|latest>');
...
for (const [key, name] of packages) {
  ...
  if (channel === 'dry-run') {
    if (registryIntegrity !== null && registryIntegrity !== localIntegrity)
      throw new Error(`PUBLISHED_ARTIFACT_DIFFERS:${specifier}`);
