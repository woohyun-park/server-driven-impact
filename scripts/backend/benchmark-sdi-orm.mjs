...
for (const key of ['SDI_POSTGRES_ADMIN_URL', 'SDI_POSTGRES_RUNTIME_URL']) {
  if (!process.env[key] || !['127.0.0.1', 'localhost', '::1'].includes(new URL(process.env[key]).hostname)) throw new Error('ISOLATED_LOCAL_BENCHMARK_REQUIRED');
}
const samples = Number(process.env.SDI_BENCHMARK_SAMPLES ?? 15);
if (!Number.isInteger(samples) || samples < 10 || samples > 1000) throw new Error('BENCHMARK_REQUIRES_10_TO_1000_SAMPLES');
...
class InstrumentedClient extends pg.Client {
  query(...args) {
    ...
    if (sample) {
      ...
      sample.sqlCalls++;
      if (/^\s*select\b/i.test(text) && new RegExp(`\\bfrom\\s+"?${schema}"?\\.`, 'i').test(text)) sample.businessSelects++;
    }
    return super.query(...args);
  }
}
const pool = new pg.Pool({ connectionString: process.env.SDI_POSTGRES_RUNTIME_URL, max: 4, Client: InstrumentedClient });
...
if (withPrisma) definitions.push(
...
const percentile = (values, fraction) => Number([...values].sort((a, b) => a - b)[Math.ceil(values.length * fraction) - 1].toFixed(3));
const summarize = records => ({
  p50Ms: percentile(records.map(r => r.durationMs), .5),
  p95Ms: percentile(records.map(r => r.durationMs), .95),
  connectionHoldP50Ms: percentile(records.map(r => r.connectionHoldMs), .5),
  connectionHoldP95Ms: percentile(records.map(r => r.connectionHoldMs), .95),
...
try {
  if (withPrisma) {
    ...
    writeFileSync(resolve(generatedDirectory, 'schema.prisma'), `generator client {
...
`);
    execFileSync(process.execPath, ['node_modules/prisma/build/index.js', 'generate', '--schema', resolve(generatedDirectory, 'schema.prisma')], { stdio: 'pipe' });
  ...
  for (const definition of definitions) {
    await admin.query(`CREATE TABLE "${schema}".${definition.table}(id integer PRIMARY KEY,value integer NOT NULL); GRANT SELECT,INSERT,UPDATE,DELETE ON "${schema}".${definition.table} TO routine_runtime`);
    const table = pgSchema(schema).table(definition.table, { id: integer('id').primaryKey(), value: integer('value').notNull() });
    ...
    if (definition.observed) {
      const resources = { rows: { schema, table: definition.table, idColumn: 'id', scopeColumn: null, columns: ['id', 'value'] } };
      const queries = { detail: { input: { parse: value => value }, plan: q.select('rows', { where: [q.eq('id', q.input('id'))] }) } };
      await admin.query(generateObserverMigration(resources, compileManifest(queries, resources), { runtimeRole: 'routine_runtime' }));
      const adapter = definition.kind === 'native' ? pgAdapter({ database: pool })
        : definition.kind === 'drizzle' ? drizzleAdapter({ database: pool })
      ...
      definition.run = () => engine.command({ scope: 'benchmark' }, async db => {
        if (definition.kind === 'prisma') return (await db.observedRow.updateMany({ data: { value: { increment: 1 } } })).count;
        const result = definition.kind === 'native' ? await db.query(nativeStatement.sql, nativeStatement.params) : await change(db);
      ...
      definition.run = async () => {
        ...
        try {
          return await client.$transaction(async tx => (await tx.plainRow.updateMany({ data: { value: { increment: 1 } } })).count, { isolationLevel: 'RepeatableRead' });
      ...
      definition.run = async () => {
        ...
        try {
          ...
          const result = definition.kind === 'native' ? await client.query(nativeStatement.sql, nativeStatement.params) : await change(drizzle(client));
  ...
  for (const rows of [1, 1000, 10000]) {
    for (const definition of definitions) {
      ...
      await admin.query(`TRUNCATE "${schema}".${definition.table}; INSERT INTO "${schema}".${definition.table} SELECT n,0 FROM generate_series(1,${rows}) n`);
    ...
    for (const definition of definitions) {
      ...
      const verified = await admin.query(`SELECT count(*)::int AS count,min(value)::int AS min,max(value)::int AS max FROM "${schema}".${definition.table}`);
  ...
  for (const definition of definitions) {
    ...
    await admin.query(`TRUNCATE "${schema}".${definition.table}; INSERT INTO "${schema}".${definition.table} VALUES(1,0)`);
    await measure(definition);
    const records = [];
    ...
    results.push({ mode: definition.name, rows: 1, concurrency: 4, samples: records.length, retries, elapsedMs: Number(elapsedMs.toFixed(3)), commandsPerSecond: Number((records.length * 1000 / elapsedMs).toFixed(1)), requestP50Ms: percentile(requestDurations, .5), requestP95Ms: percentile(requestDurations, .95), ...summarize(records) });
  ...
  const report = {
    ...
    isolation: 'repeatable read', poolSize: 4, samples,
    notes: [
