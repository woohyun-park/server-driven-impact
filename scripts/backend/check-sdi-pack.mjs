...
for (const directory of packageDirectories) {
  ...
  if (useExistingArtifacts) {
    ...
    const packed = JSON.parse(
      run('pnpm', ['pack', '--json', '--pack-destination', destination], resolve(root, 'packages', directory), {
        ...process.env,
    ...
    if (basename(result.filename) !== filename)
      throw new Error(`UNEXPECTED_ARCHIVE_NAME:${directory}:${result.filename}`);
...
await writeFile(
  resolve(destination, 'consumer.ts'),
  `import {calculateImpact} from '@server-driven-impact/core';
...
if (verifyPostgres) {
  if (!process.env.SDI_POSTGRES_ADMIN_URL || !process.env.SDI_POSTGRES_RUNTIME_URL)
    throw new Error('PACK_POSTGRES_ENV_REQUIRED');
  for (const driver of ['postgres', 'pg']) {
    console.log(
      run('node', ['out/postgres-demo.js'], destination, { ...process.env, SDI_POSTGRES_DRIVER: driver }).trim(),
    );
  }
}
...
for (const directory of packageDirectories) {
  const manifest = JSON.parse(
    await readFile(
      resolve(destination, 'node_modules/@server-driven-impact', directory.slice(4), 'package.json'),
      'utf8',
    ),
  );
  ...
  if (manifest.version !== releaseVersion)
    throw new Error(`SDI_VERSION_MISMATCH:${manifest.name}:${manifest.version}:${releaseVersion}`);
}
if (Object.keys(manifests['@server-driven-impact/core'].dependencies).length)
  throw new Error('CORE_HAS_RUNTIME_DEPENDENCIES');
if (
  Object.keys(manifests['@server-driven-impact/runtime'].dependencies).sort().join(',') !==
  '@server-driven-impact/cache-contract,@server-driven-impact/core'
)
  throw new Error('RUNTIME_DEPENDENCY_LEAK');
if (!manifests['@server-driven-impact/tanstack-query'].peerDependencies['@tanstack/query-core'])
  throw new Error('TANSTACK_QUERY_PEER_REQUIRED');
...
if ('@pgsql/parser' in manifests['@server-driven-impact/sqlite'].dependencies)
  throw new Error('SQLITE_POSTGRES_DEPENDENCY_LEAK');
for (const adapter of ['@server-driven-impact/postgres', '@server-driven-impact/sqlite']) {
  if (
    !manifests[adapter].peerDependencies['@server-driven-impact/core'] ||
    !manifests[adapter].peerDependencies['@server-driven-impact/runtime']
  ) {
...
for (const peer of optionalOrmPeers) {
  ...
  if (
    !postgresManifest.peerDependencies[peer] ||
    postgresManifest.peerDependenciesMeta[peer]?.optional !== true ||
    peer in postgresManifest.dependencies
  ) {
  ...
  for (const packageName of [
    ...
    if (peer in manifests[packageName].dependencies || peer in manifests[packageName].peerDependencies)
      throw new Error(`ORM_DEPENDENCY_LEAK:${packageName}:${peer}`);
  }
}
async function assertNotInstalled(directory, packages) {
  for (const packageName of packages) {
    try {
      await access(resolve(directory, 'node_modules', packageName));
    } catch (error) {
...
async function isolatedConsumer(name, dependencies, sourceFiles, execute, compilerOptions = {}) {
  ...
  await assertNotInstalled(
    directory,
    optionalOrmPeers.filter(peer => !(peer in dependencies)),
  );
  return directory;
}

await isolatedConsumer(
  'core-only',
...
const browserConsumer = await isolatedConsumer(
  'browser-types',
  {
    '@server-driven-impact/core': archives['sdi-core'],
    '@server-driven-impact/cache-contract': archives['sdi-cache-contract'],
...
await assertNotInstalled(browserConsumer, [
  '@server-driven-impact/runtime',
...
await isolatedConsumer(
  'runtime-only',
  {
    '@server-driven-impact/core': archives['sdi-core'],
    '@server-driven-impact/cache-contract': archives['sdi-cache-contract'],
    '@server-driven-impact/runtime': archives['sdi-runtime'],
  },
  {
...
const sqliteOnly = await isolatedConsumer(
  'sqlite-only',
  {
    '@server-driven-impact/core': archives['sdi-core'],
    '@server-driven-impact/cache-contract': archives['sdi-cache-contract'],
    '@server-driven-impact/runtime': archives['sdi-runtime'],
    '@server-driven-impact/sqlite': archives['sdi-sqlite'],
  },
  {
...
try {
  await access(resolve(sqliteOnly, 'node_modules/@pgsql/parser'));
  throw new Error('SQLITE_INSTALLED_POSTGRES_PARSER');
} catch (error) {
...
await isolatedConsumer(
  'postgres-js-only',
  {
    ...
    '@server-driven-impact/cache-contract': archives['sdi-cache-contract'],
    '@server-driven-impact/runtime': archives['sdi-runtime'],
    '@server-driven-impact/postgres': archives['sdi-postgres'],
    postgres: '3.4.8',
  },
  {
...
const integrity = Object.fromEntries(
  await Promise.all(
    Object.entries(archives).map(async ([directory, filename]) => {
...
const overrides = Object.fromEntries(
  Object.entries(archives).map(([directory, filename]) => [
    '@server-driven-impact/' + directory.slice(4),
    'file:' + resolve(artifactDirectory, filename),
  ]),
);
await writeFile(resolve(artifactDirectory, 'pnpm-overrides.json'), JSON.stringify(overrides, null, 2) + '\n');
const source = {
  revision: run('git', ['rev-parse', 'HEAD'], root).trim(),
