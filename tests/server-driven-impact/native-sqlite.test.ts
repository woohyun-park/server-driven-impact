...
function fixture() {
  ...
  database.exec('pragma foreign_keys=on; create table rows(id text primary key, customer text); create table notes(id text primary key, body text)');
  ...
  return {database,resources,queries,engine:createImpact({adapter:sqliteAdapter({database}),resources,queries})};
}
describe('SQLite native statement lifecycle and collection', () => {
  it('preserves synchronous run/get/all results and closes escaped statements and savepoint children', async () => {
    const {engine,database} = fixture(); await engine.validate();
    ...
    const result = await engine.command({scope:null}, async tx => {
      ...
      await expect(tx.savepoint(async inner => {
        child = inner.prepare('insert into rows values(?,?)'); child.run('rolled','b');
    ...
    expect(result.impact.targets.find(t=>t.endpoint==='list')?.selector).toEqual({kind:'inputs',values:[{customer:'a'}]});
  ...
  it('prevents observer mutation and shared-engine observer drift', async () => {
    const {engine,database,resources} = fixture(); await engine.validate();
    await expect(engine.command({scope:null},tx=>tx.execute('delete from temp.sdi_observed_facts'))).rejects.toThrow('SQLITE_OBSERVER_ACCESS_FORBIDDEN');
    const byId = createImpact({adapter:sqliteAdapter({database}),resources,queries:{id:{input:{parse:v=>v as Record<string,unknown>},plan:q.select('rows',{where:[q.eq('id',q.input('id'))]})}}});
    await byId.validate();
    const first = await engine.command({scope:null}, async tx => tx.prepare('insert into rows values(?,?)').run('one','customer'));
    expect(first.impact.targets.find(t=>t.endpoint==='list')?.selector).toEqual({kind:'inputs',values:[{customer:'customer'}]});
    const second = await byId.command({scope:null}, async tx => tx.prepare('update rows set customer=? where id=?').run('next','one'));
  ...
  it('rejects UPDATE OR REPLACE before a conflicting row can disappear without an observer event', async () => {
    ...
    await expect(engine.command({scope:null},tx=>tx.execute("update or replace rows set id='2' where id='1'"))).rejects.toThrow('SQLITE_REPLACE_UNSUPPORTED');
    await expect(engine.command({scope:null},async tx=>tx.prepare("with target as (select '1' as id) update /* conflict policy */ or replace rows set id='2' where id in (select id from target)").run())).rejects.toThrow('SQLITE_REPLACE_UNSUPPORTED');
    expect(database.prepare('select * from rows order by id').all()).toEqual([{id:'1',customer:'A'},{id:'2',customer:'B'}]);
  });
  it('keeps independent resource selectors narrow when a large batch overflows', async () => {
    const {engine} = fixture(); await engine.validate();
    const result = await engine.command({scope:null}, async tx => {
    ...
    expect(matchesInputSelector({customer:'many'},result.impact.targets.find(t=>t.endpoint==='list')!.selector)).toBe(true);
  });
});
