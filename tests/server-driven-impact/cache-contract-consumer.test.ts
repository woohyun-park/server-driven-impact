...
import { buildQueryKey, prepareCacheQuery, buildQueryExecutionInput, compileCacheInvalidations, defineCacheContract, validateCacheContractCoverage, type CacheContract, type CacheDiagnostic, type CacheInputField } from '@server-driven-impact/cache-contract';
import { applyCacheInvalidations } from '@server-driven-impact/tanstack-query';
import { createImpact, defineQueries, q, type CommandOptions, type CommandResult, type CommandInvalidationResult } from '@server-driven-impact/runtime';
...
function rpcContract(input: Record<string, CacheInputField> = {userId:{type:'string',required:true}}) {
  return defineCacheContract({
    id:'rpc',version:1,queries:(['query','infinite'] as const).map(kind => ({
      operationId:kind,endpoint:'profile.getProfileUserInfo',kind,input,
      ...(kind === 'infinite' ? {pageInput:{cursor:{type:'string' as const,nullable:true}}} : {}),
      key:{template:{kind:'array' as const,items:[
        {kind:'literal' as const,value:path},
        {kind:'object' as const,fields:{input:{kind:'inputs' as const},type:{kind:'literal' as const,value:kind}}},
      ]}},
      fallback:[path,{type:kind}],
    })),
  });
}
function impact(value: Record<string, string | number | boolean | null>, endpoint = 'profile.getProfileUserInfo'): ImpactSet {
  return {protocolVersion:1,targets:[{endpoint,scope:'caller',selector:{kind:'inputs',values:[value]}}]};
}
const all: ImpactSet = {protocolVersion:1,targets:[{endpoint:'profile.getProfileUserInfo',scope:'caller',selector:{kind:'all'}}]};
...
describe('generic nested cache contracts', () => {
  it('generates actual oRPC query/infinite keys without an oRPC runtime dependency', () => {
    ...
    for (const kind of ['query','infinite'] as const) {
      expect(buildQueryKey(contract,kind,{userId:'user-a'})).toEqual(generateOperationKey(path,{input:{userId:'user-a'},type:kind}));
    }
    expect(() => defineCacheContract({...contract,queries:contract.queries.map(query => ({...query,key:contract.queries[0].key,fallback:[path]}))})).toThrow('CACHE_KEY_COLLISION');
  });
  it('retains structured values and normalizes ordered vs set inputs explicitly', () => {
    const input: Record<string, CacheInputField> = {
      ...
      filters:{type:'object',properties:{uid:{type:'object',properties:{in:{type:'array',items:{type:'string'}}}},isActive:{type:'boolean'}},required:true},
      optional:{type:'string',nullable:true},
    };
    const contract = rpcContract(input);
    const raw = {missionIds:['b','a','b'],targetUserIds:['b','a','b'],filters:{uid:{in:['user-a']},isActive:true}};
    ...
    expect(() => buildQueryKey(contract,'query',{...raw,missionIds:[undefined]})).toThrow('CACHE_ARRAY_ITEM_REQUIRED');
  });
  it('separates infinite identity from execution-only page inputs', () => {
    const contract = rpcContract();
    const key = buildQueryKey(contract,'infinite',{userId:'user-a'});
    expect(buildQueryExecutionInput(contract,'infinite',{userId:'user-a'},{cursor:'page-2'})).toEqual({userId:'user-a',cursor:'page-2'});
    expect(buildQueryKey(contract,'infinite',{userId:'user-a'})).toEqual(key);
    expect(() => buildQueryKey(contract,'infinite',{userId:'user-a',cursor:'page-2'})).toThrow('UNKNOWN_CACHE_INPUT');
    expect(() => buildQueryExecutionInput(contract,'query',{userId:'user-a'},{})).toThrow('CACHE_PAGE_INPUT_REQUIRES_INFINITE');
    const diagnostics: CacheDiagnostic[] = [];
    compileCacheInvalidations(contract,impact({cursor:'page-2'}),'tenant',{explain:value => diagnostics.push(value)});
  ...
  it('supports endpoint/domain prefixes and preserves query/infinite isolation', async () => {
    ...
    const keys = [buildQueryKey(contract,'query',{userId:1}),buildQueryKey(contract,'query',{userId:2}),buildQueryKey(contract,'infinite',{userId:1})];
    ...
    const domain = defineCacheContract({...contract,queries:contract.queries.map(query => ({...query,fallback:[['data','session','profile']]}))});
    expect(compileCacheInvalidations(domain,all,'tenant').invalidations).toEqual([{queryKey:[['data','session','profile']],exact:false}]);
    expect(() => defineCacheContract({...contract,queries:[{...contract.queries[0],fallback:[['unrelated']]}]})).toThrow('CACHE_FALLBACK_NOT_PROVEN');
    client.clear();
  });
  it('requires explicit coverage and reports unknown impact endpoints', () => {
    const contract = rpcContract();
    const endpoints = [{endpoint:'profile.getProfileUserInfo',cache:'cacheable' as const},{endpoint:'clock',cache:'no-store' as const},{endpoint:'admin',cache:'cacheable' as const}];
    expect(() => validateCacheContractCoverage(contract,endpoints)).toThrow('CACHE_ENDPOINT_UNCOVERED:clock');
    const covered = defineCacheContract({...contract,excludedEndpoints:[{endpoint:'clock',reason:'no-store'},{endpoint:'admin',reason:'not-consumed'}]});
    ...
    expect(() => compileCacheInvalidations(covered,impact({},'typo'),'tenant')).toThrow('CACHE_ENDPOINT_UNCOVERED:typo');
    expect(() => validateCacheContractCoverage(contract,[{endpoint:'profile.getProfileUserInfo',cache:'no-store'}])).toThrow('CACHE_ENDPOINT_REQUIRES_NO_STORE');
  });
  it('explains widening without exposing values and respects the response budget', () => {
    ...
    const result = compileCacheInvalidations(contract,impact({userId:'private-user-id'}),'tenant',{explain:value => diagnostics.push(value)});
    ...
    const many: ImpactSet = {protocolVersion:1,targets:[{endpoint:'profile.getProfileUserInfo',scope:'caller',selector:{kind:'inputs',values:[{userId:1},{userId:2}]}}]};
    diagnostics.length = 0;
    expect(compileCacheInvalidations(numeric,many,'tenant',{maxInvalidations:2,explain:value => diagnostics.push(value)}).invalidations).toHaveLength(2);
    expect(diagnostics.some(value => value.reason === 'budget')).toBe(true);
    expect(() => compileCacheInvalidations(numeric,all,'tenant',{maxBytes:1})).toThrow('CACHE_INVALIDATION_LIMIT');
  });
  it('never excludes a protocol-1 matching input across omission, defaults and scalar coercion', () => {
    const fixtures: {field:CacheInputField;values:unknown[];selectors:(string|number|boolean|null)[]}[] = [
      {field:{type:'string',required:true},values:['x','X','x ','1','01','true'],selectors:['x','X','x ',1,true]},
      ...
      {field:{type:'string',required:true,enum:['a','A','b']},values:['a','A','b'],selectors:['a','b']},
    ];
    for (const fixture of fixtures) {
      ...
      for (const expected of fixture.selectors) {
        ...
        for (const value of fixture.values) {
          ...
          expect(result.invalidations.some(filter => matchQuery(filter,query)),JSON.stringify({fixture,expected,input})).toBe(true);
          client.clear();
        }
      }
    }
  ...
  it('does not overlook optional secondary params even with an anchor', () => {
    const contract = defineCacheContract({id:'flat',version:1,queries:[{
      operationId:'list',endpoint:'profile.getProfileUserInfo',kind:'query',
      input:{id:{type:'number'},page:{type:'number'}},
      key:{prefix:['list'],params:['id','page'],paramsAnchor:['id'],omitEmptyParams:true},fallback:['list'],
    }]});
    expect(compileCacheInvalidations(contract,impact({id:1,page:2}),'tenant').invalidations).toEqual([{queryKey:['list'],exact:false}]);
  });
});

describe('verified string cache precision', () => {
  function profileContract() {
    return defineCacheContract({id:'profiles',version:2,queries:[{
      operationId:'profileByUsername',endpoint:'profiles.byUsername',kind:'query' as const,
      input:{username:{type:'string' as const,required:true}},
      key:{prefix:['profiles','byUsername'],path:['username']},fallback:['profiles','byUsername'],
    }]});
  }
  function profileEngine(database: DatabaseSync, options: {explain?: (value:CacheDiagnostic)=>void; contract?:CacheContract} = {}) {
    ...
    return createImpact({
      resources:{profiles:{schema:'main',table:'profiles',idColumn:'id',scopeColumn:null,columns:['id','username','birthday']}},
      queries,adapter:sqliteAdapter({database}),cacheContracts:[options.contract ?? profileContract()],
  ...
  it('invalidates only OLD and NEW keys after SQLite verifies direct TEXT/BINARY equality', async () => {
    ...
    database.exec('pragma foreign_keys=on; create table profiles(id text primary key,username text not null,birthday text not null)');
    const engine = profileEngine(database);
    await engine.validate();
    await engine.command({scope:null},db=>db.execute('insert into profiles values(?,?,?)',['1','홍길동','2000-01-01']),{cacheContract:{id:'profiles',version:2}});
    const birthday = await engine.command({scope:null},db=>db.execute('update profiles set birthday=? where id=?',['2001-01-01','1']),{cacheContract:{id:'profiles',version:2}});
    expect(birthday.cacheInvalidation.invalidations).toEqual([{queryKey:['profiles','byUsername','홍길동'],exact:true}]);
    const renamed = await engine.command({scope:null},db=>db.execute('update profiles set username=? where id=?',['홍길순','1']),{cacheContract:{id:'profiles',version:2}});
  ...
  it('keeps case, spaces, numeric-looking strings and Unicode forms distinct under BINARY equality', async () => {
    ...
    const result=await engine.command({scope:null},db=>db.execute('update profiles set birthday=? where id=?',['2001-01-01','0']),{cacheContract:{id:'profiles',version:2}});
    expect(result.cacheInvalidation.invalidations).toEqual([{queryKey:['profiles','byUsername','Alice'],exact:true}]);
    database.close();
  });

  it('keeps endpoint fallback before validation and for non-BINARY comparison', async () => {
    for (const fixture of [
      {schema:'create table profiles(id text primary key,username text not null,birthday text not null)',validate:false},
      {schema:'create table profiles(id text primary key,username text collate nocase not null,birthday text not null)',validate:true},
    ] as const) {
      const diagnostics:CacheDiagnostic[]=[];
      const database = new DatabaseSync(':memory:');
      ...
      const result=await engine.command({scope:null},db=>db.execute('insert into profiles values(?,?,?)',['1','Alice','2000-01-01']),{cacheContract:{id:'profiles',version:2}});
  ...
  it('rejects a parser that changes a verified string before querying', async () => {
    const database = new DatabaseSync(':memory:');
    database.exec('pragma foreign_keys=on; create table profiles(id text primary key,username text not null,birthday text not null)');
    const queries=defineQueries({'profiles.byUsername':{
      input:{parse:(value:unknown)=>({username:String((value as {username:unknown}).username).toLowerCase()})},
      plan:q.select('profiles',{where:[q.eq('username',q.input('username'))]}),
    }});
    const engine=createImpact({
      resources:{profiles:{schema:'main',table:'profiles',idColumn:'id',scopeColumn:null,columns:['id','username','birthday']}},
      queries,adapter:sqliteAdapter({database}),cacheContracts:[profileContract()],
    });
    await engine.validate();
    await expect(engine.query('profiles.byUsername',{username:'Alice'},{scope:null})).rejects.toThrow('QUERY_INPUT_PRESERVATION_VIOLATION:username');
    database.close();
  });
});
...
describe('post-commit runtime contract', () => {
  function engine(fail: 'impact' | 'cache' | undefined, onCommit: () => void = () => {}) {
    const adapter: ImpactAdapter<null> = {[bindAdapter]:() => ({
      validate:async () => {},query:async (_scope,work) => work(async () => []),
      command:async (_scope,writes,work) => {
        const data = await work(null);
        onCommit();
        if (fail === 'impact') writes.snapshot = () => {throw new Error('CALCULATION_FAILED');};
        return data;
      },
    })};
    return createImpact({
      resources:{items:{schema:'main',table:'items',idColumn:'id',scopeColumn:null,columns:['id']}},
      queries:{'profile.getProfileUserInfo':{input:{parse:(value:unknown) => value},plan:q.select('items')}},
      adapter,cacheContracts:[rpcContract()],cacheInvalidationOptions:fail === 'cache' ? {maxBytes:1} : {},
    });
  }
  it.each(['impact','cache'] as const)('preserves business data after %s failure without re-executing work', async fail => {
    let commits = 0, calls = 0;
    const runtime = engine(fail,() => commits++);
    const businessData = {ok:true,id:'created'};
    const error = await runtime.command({scope:'tenant'},async () => {calls++;return businessData;},{cacheContract:scope.contract}).catch(error => error);
  ...
  it('snapshots scope and version before awaiting command work', async () => {
    const reference = {id:'rpc',version:1}, context = {scope:'tenant'};
    const options = {cacheContract:reference};
    const result = await engine(undefined).command(context,async () => {
      reference.version = 2;context.scope = 'different';
  ...
  it('infers cache output for explicit options and a union for dynamic options', async () => {
    ...
    expectTypeOf<Awaited<ReturnType<typeof invoke>>>().toEqualTypeOf<CommandResult<{id:number}> | CommandInvalidationResult<{id:number}>>();
  ...
  it('retains impact failure classification in the two-argument command', async () => {
    ...
    const error = await engine('impact').command({scope:'tenant'},async () => data).catch(error => error);
    expect(error).toBeInstanceOf(ImpactUnavailableError);
...
describe('executor lifecycle boundaries', () => {
  it('waits for application callbacks and does not apply responses to a retired session', async () => {
    ...
    const result = consumeCommittedReply({
      response:Promise.resolve({commitState:'committed',data:'saved',cacheInvalidation:compileCacheInvalidations(rpcContract(),all,'tenant')}),
      queryClient:client,...scope,isCurrentSession:() => current,
      afterCommit:async () => {callbackStarted();await new Promise<void>(resolve => {release=resolve;});},
    });
    await started;
  ...
  it('continues cache synchronization after a failed post-commit callback', async () => {
    ...
    const result = await consumeCommittedReply({
      response:Promise.resolve({commitState:'committed',data:'saved',cacheInvalidation:compileCacheInvalidations(rpcContract(),all,'tenant')}),
      queryClient:client,...scope,isCurrentSession:() => true,afterCommit:async () => {throw failure;},
    });
  ...
  it.each([false,true])('initial fetch overlap: cancelInFlight=%s', async cancelInFlight => {
    ...
    let calls = 0;
    const observer = new QueryObserver(client,{queryKey:key,queryFn:() => ++calls === 1 ? new Promise<string>(resolve => {resolveOld = resolve;}) : Promise.resolve('fresh')});
    const unsubscribe = observer.subscribe(() => {});
    const pending = applyCacheInvalidations(client,compileCacheInvalidations(rpcContract(),all,'tenant'),{...scope,cancelInFlight});
    resolveOld('old');
    await pending;
  ...
  it('propagates refetch failure without representing a failed server commit', async () => {
    ...
    const key = buildQueryKey(rpcContract(),'query',{userId:'user-a'});
    client.setQueryData(key,'cached');
    const observer = new QueryObserver(client,{queryKey:key,staleTime:Infinity,queryFn:async () => {throw new Error('READ_OFFLINE');}});
    const unsubscribe = observer.subscribe(() => {});
    const committedResponse = {data:{saved:true},cacheInvalidation:compileCacheInvalidations(rpcContract(),all,'tenant')};
    await expect(applyCacheInvalidations(client,committedResponse.cacheInvalidation,scope)).rejects.toThrow('READ_OFFLINE');
