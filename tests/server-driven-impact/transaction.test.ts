...
it('engine response waits for the adapter commit and propagates commit rejection', async () => {
  ...
  const pending = engine
    .command(adapter, { scope: null }, async () => 42)
    .then(result => {
      resolved = true;
      return result;
    });
  ...
  release();
  expect((await pending).data).toBe(42);
  const failing: CommandAdapter<object> = {
    async command(_scope, work) {
      await work({}, new WriteSet());
...
it('uses the execution scope snapshot when the caller mutates context before commit', async () => {
      ...
      writes.add([
        {
          ...
          operation: 'insert',
          before: { kind: 'absent' },
          after: { kind: 'known', scope, fields: { id: 'one' } },
          changedColumns: null,
        },
      ]);
      return data;
  ...
  const result = await scoped.command(adapter, context, async () => {
    context.scope = 'b';
    return 'saved';
  });
