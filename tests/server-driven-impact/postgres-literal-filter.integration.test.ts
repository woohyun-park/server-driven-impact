...
describe.skipIf(!enabled)('PostgreSQL certified literal equality filters',()=>{
  ...
  const resources:Resources={records:{schema,table:'records',idColumn:'id',scopeColumn:null,columns:['id','status','customer','enabled','qty','ratio','day']}};
  const filtered=(column:string,literal:string|number)=>({input,plan:q.select('records',{columns:['id'],where:[q.eq(column,q.literal(literal))]})});
  const queries={
    ready:{input,plan:q.select('records',{columns:['id'],where:[q.eq('status',q.literal('ready')),q.eq('customer',q.input('customer'))]})},
    boolean:filtered('enabled',1),integer:filtered('qty','01'),float:filtered('ratio',0.10000000149011612),date:filtered('day','2026-01-01T00:00:00.000Z'),
  };
  ...
  it('prunes unrelated text values but keeps type coercion, float rounding and date normalization safe',async()=>{
    ...
    for(const endpoint of ['boolean','integer','float','date'] as const)expect(await engine.query(endpoint,{},context)).toEqual([]);
    const writes=new WriteSet();
    await bound.command(null,writes,tx=>tx.execute(sql`insert into ${identifier(schema)}.records values('one','draft','a',true,1,0.100000001,'2026-01-01')`));
    ...
    for(const endpoint of ['integer','float','date'] as const)expect(await engine.query(endpoint,{},context),endpoint).toEqual([{id:'one'}]);
    expect(await engine.query('ready',{customer:'a'},context)).toEqual([]);
    const unrelated=await engine.command(context,tx=>tx.execute(sql`update ${identifier(schema)}.records set status='archived' where id='one'`));
    expect(unrelated.impact.targets).toEqual([]);
    const entered=await engine.command(context,tx=>tx.execute(sql`update ${identifier(schema)}.records set status='ready' where id='one'`));
    expect(await engine.query('ready',{customer:'a'},context)).toEqual([{id:'one'}]);
    expect(entered.impact.targets).toEqual([{endpoint:'ready',scope:'global',selector:{kind:'inputs',values:[{customer:'a'}]}}]);
    const left=await engine.command(context,tx=>tx.execute(sql`update ${identifier(schema)}.records set status='draft' where id='one'`));
    expect(await engine.query('ready',{customer:'a'},context)).toEqual([]);
