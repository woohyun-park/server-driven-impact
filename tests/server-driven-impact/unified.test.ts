...
const order = (id: string, customer = 'first') => ({
  id,
  tenant_id: 'a',
...
async function insert(db: SqliteCommandDb, table: string, rows: Record<string, unknown>[], returning = false) {
  ...
  return db.execute(
    `insert into ${table}(${names.join(',')}) values ${tuples}${returning ? ' returning *' : ''}`,
    values,
  );
}
const update = (
  ...
  set: string,
  where: string,
  values: readonly (null | string | number)[] = [],
) => db.execute(`update ${table} set ${set} where ${where} returning *`, values);
...
describe('unified API / actual SQLite', () => {
  ...
  it('runs the same executable definitions for reads and impact, without a supplied manifest', async () => {
    ...
    const item = await engine.command(context, db =>
      insert(db, 'order_items', [{ id: 'item', tenant_id: 'a', order_id: 'one', amount: 42 }]),
    );
  ...
  it('discards rolled back facts, rejects leaked contexts, and reports no-op correctly', async () => {
    ...
    await expect(
      engine.command(context, async db => {
        captured = db;
    ...
    const result = await engine.command(context, async db => {
      await expect(
        db.savepoint(async child => {
          await insert(child, 'orders', [order('discarded', 'discarded')]);
  ...
  it('commit failure publishes no result, and a caught failed write cannot commit earlier writes', async () => {
    ...
    database.exec(
      'drop table order_items; create table order_items(id text primary key,tenant_id text not null,order_id text references orders(id) on delete cascade deferrable initially deferred,amount integer not null)',
    );
    const second = createImpact({ adapter: sqliteAdapter({ database }), resources, queries });
    await expect(
      second.command(context, db =>
        insert(db, 'order_items', [{ id: 'bad', tenant_id: 'a', order_id: 'missing', amount: 2 }]),
      ),
  ...
  it('rejects overlapping savepoints and unfinished writes', async () => {
    ...
    await expect(
      engine.command(context, async db => {
        void db.savepoint(async child => {
          await new Promise(resolve => setTimeout(resolve, 5));
          await insert(child, 'orders', [order('unawaited')]);
        });
      }),
  ...
  it('bounds bulk facts and keeps OLD/NEW nullable selector membership', async () => {
    ...
    const moved = await engine.command(context, db =>
      update(db, 'orders', 'customer_id=?', 'customer_id is null', ['new']),
    );
    ...
    await engine.command(context, db =>
      insert(
        db,
        'orders',
        Array.from({ length: 210 }, (_, i) => order('bulk' + i)),
      ),
    );
  ...
  it('preserves every changed resource when one command exceeds the fact budget', async () => {
    ...
    const result = await engine.command(context, async db => {
      await insert(
        db,
        'orders',
        Array.from({ length: 201 }, (_, i) => order('overflow-' + i)),
      );
  ...
  it('executes count, OR/NOT and input pagination in the database', async () => {
    ...
    const queries = defineQueries({
      count: {
        input,
        plan: q.count('orders', {
          where: [q.or(q.eq('status', q.literal('ready')), q.not(q.eq('customer_id', q.literal('blocked'))))],
        }),
      },
      page: {
        input,
        plan: q.select('orders', {
          ...
          order: [{ field: 'id' }],
          limit: q.input('size'),
          offset: q.input('offset'),
        }),
      },
      offsetOnly: {
        input,
        plan: q.select('orders', { columns: ['id'], order: [{ field: 'id' }], offset: q.input('offset') }),
      },
    });
  ...
  it('includes unexecuted branches and snapshots the registered plans', async () => {
    ...
    const queries = defineQueries({
      sample: { input: { parse: () => ({}) }, plan: q.when(() => false, q.select('items'), plan) },
    });
    const engine = createImpact({ adapter, resources, queries });
    plan.resource = 'items';
  ...
  it('rejects missing adapters, arbitrary manifests, unknown endpoints and SQL injection', async () => {
    ...
    await expect(
      engine.command(context, db =>
        db.execute('update orders set status=? where id=?; drop table orders', ['bad', 'x']),
      ),
    ).rejects.toThrow('SQLITE_SINGLE_STATEMENT_REQUIRED');
  });
  it('observes native SQLite DML and writes performed by business triggers', async () => {
    ...
    database.exec(
      "create trigger hidden after insert on orders begin update orders set note='hidden' where id=new.id; end",
    );
    const engine = createImpact({ adapter, resources, queries });
  ...
  it('observes native identity changes and rejects operations it cannot observe safely', async () => {
    ...
    const moved = await engine.command(context, db =>
      db.execute('update orders set id=? where id=? returning *', ['new', 'old']),
    );
    ...
    await expect(engine.command(context, db => db.execute('commit'))).rejects.toThrow(
      'SQLITE_TRANSACTION_OR_DDL_FORBIDDEN',
    );
    await expect(engine.command(context, db => db.execute('/* harmless-looking prefix */ COMMIT'))).rejects.toThrow(
      'SQLITE_TRANSACTION_OR_DDL_FORBIDDEN',
    );
    await expect(
      engine.command(context, db =>
        db.execute(
          "insert into orders(id,tenant_id,customer_id,status,priority,note) values('escape','a','first','ready',1,null); commit",
        ),
      ),
    ).rejects.toThrow('SQLITE_SINGLE_STATEMENT_REQUIRED');
    await expect(
      engine.command(context, db => db.execute("update orders set note='contains;semicolon' where id='new'")),
    ).resolves.toBeDefined();
    await expect(
      engine.command(context, db =>
        db.execute(
          "insert or replace into orders(id,tenant_id,customer_id,status,priority,note) values('new','a','second','ready',1,null)",
        ),
      ),
  ...
  it('matches numeric affinity and NOCASE result changes conservatively', async () => {
    ...
    databases.push(database);
    database.exec(
      'create table values_table(id text primary key,tenant text,number_value integer,label text collate nocase)',
    );
    const resources = {
      values: {
        schema: 'main',
        ...
        idColumn: 'id',
        scopeColumn: 'tenant',
        columns: ['id', 'tenant', 'number_value', 'label'],
      },
    };
    const input = { parse: (value: unknown) => value as Record<string, unknown> };
    const queries = defineQueries({
    ...
    const inserted = await engine.command(context, db =>
      insert(db, 'values_table', [{ id: 'one', tenant: 'a', number_value: 1, label: 'work' }]),
    );
