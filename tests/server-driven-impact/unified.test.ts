...
import { createImpact, q, defineQueries, type CommandDb } from '@server-driven-impact/runtime';
...
describe('unified API / actual SQLite', () => {
  it('runs the same executable definitions for reads and impact, without a supplied manifest', async () => {
    ...
    const inserted = await engine.command(context, db => db.insert('orders', [order('one')]));
    ...
    const moved = await engine.command(context, db => db.update('orders', { where: { id: 'one' }, set: { customer_id: 'second' } }));
    for (const customer of ['first', 'second']) expect(includes(moved.impact, 'orders.list', { customer })).toBe(true);
    const item = await engine.command(context, db => db.insert('items', [{ id: 'item', tenant_id: 'a', order_id: 'one', amount: 42 }]));
  ...
  it('discards rolled back facts, rejects leaked contexts, and reports no-op correctly', async () => {
    ...
    await expect(engine.command(context, async db => {
      captured = db;
      await db.insert('orders', [order('rolled')]);
    ...
    const result = await engine.command(context, async db => {
      await expect(db.savepoint(async child => { await child.insert('orders', [order('discarded', 'discarded')]); throw new Error('child'); })).rejects.toThrow('child');
      return db.savepoint(child => child.insert('orders', [order('kept', 'kept')]));
    });
    expect(JSON.stringify(result.impact)).not.toContain('discarded');
    expect((await engine.command(context, db => db.update('orders', { where: { id: 'missing' }, set: { status: 'draft' } }))).impact.targets).toEqual([]);
    expect((await engine.command(context, db => db.update('orders', { where: { id: 'kept' }, set: { status: 'ready' } }))).impact.targets).toEqual([]);
    expect((await engine.command(context, db => db.update('orders', { where: { id: 'kept' }, set: { note: 'private' } }))).impact.targets.map(t => t.endpoint)).toEqual(['orders.detail']);
  });
  it('commit failure publishes no result, and a caught failed write cannot commit earlier writes', async () => {
    ...
    await expect(engine.command(context, async db => {
      await db.insert('orders', [{ ...order('one'), note: 'same' }]);
      await expect(db.insert('orders', [{ ...order('two'), note: 'same' }])).rejects.toThrow();
    ...
    await expect(second.command(context, db => db.insert('items', [{ id: 'bad', tenant_id: 'a', order_id: 'missing', amount: 2 }]))).rejects.toThrow(/FOREIGN KEY/);
  ...
  it('serializes commands on a shared connection and isolates request collectors', async () => {
    ...
    const results = await Promise.all(Array.from({ length: 12 }, (_, i) => engine.command(context, async db => {
      ...
      return db.insert('orders', [order(String(i), 'customer' + i)]);
    })));
  ...
  it('rejects overlapping savepoints and unfinished writes', async () => {
    const { engine } = fixture();
    await expect(engine.command(context, async db => { void db.savepoint(async child => { await new Promise(resolve => setTimeout(resolve, 5)); await child.insert('orders', [order('unawaited')]); }); })).rejects.toThrow('UNAWAITED_DATABASE_OPERATION');
  ...
  it('bounds bulk facts and keeps OLD/NEW nullable selector membership', async () => {
    const { engine } = fixture();
    await engine.command(context, db => db.insert('orders', [{ ...order('nullable'), customer_id: null }]));
    const moved = await engine.command(context, db => db.update('orders', { where: { customer_id: null }, set: { customer_id: 'new' } }));
    ...
    await engine.command(context, db => db.insert('orders', Array.from({ length: 210 }, (_, i) => order('bulk' + i))));
    const bulk = await engine.command(context, db => db.update('orders', { where: {}, set: { status: 'draft' } }));
    expect(bulk.data.count).toBe(211);
  ...
  it('preserves every changed resource when one command exceeds the fact budget',async()=>{
    ...
    const result=await engine.command(context,async db=>{
      await db.insert('orders',Array.from({length:201},(_,i)=>order('overflow-'+i)));
      await db.insert('items',[{id:'overflow-item',tenant_id:'a',order_id:'overflow-0',amount:1}]);
    });
  ...
  it('includes unexecuted branches and snapshots the registered plans', async () => {
    ...
    const changed = await engine.command(context, db => db.insert('items', [{ id: 'i', tenant_id: 'a', order_id: 'one', amount: 1 }]));
  ...
  it('rejects missing adapters, arbitrary manifests, unknown endpoints and SQL injection', async () => {
    ...
    await engine.command(context, db => db.insert('orders', [order('injection', "'; delete from orders; --")]));
    expect(await engine.query('orders.list', { customer: "'; delete from orders; --" }, context)).toHaveLength(1);
    await expect(engine.command(context, db => db.update('orders', { where: { 'id; drop table orders': 'x' }, set: { status: 'bad' } }))).rejects.toThrow('UNREGISTERED_COLUMN');
  });
  it('observes native SQLite DML and writes performed by business triggers', async () => {
    ...
    const engine=createImpact({adapter,resources,queries});
    const result=await engine.command(context,db=>db.sqlite.execute(
  ...
  it('observes native identity changes and rejects operations it cannot observe safely',async()=>{
    const {engine}=fixture();
    await engine.command(context,db=>db.insert('orders',[order('old')]));
    const moved=await engine.command(context,db=>db.sqlite.execute('update orders set id=? where id=? returning *',['new','old']));
  ...
  it('runs without implicit validation and performs fresh SQLite validation only when requested',async()=>{
    const {engine,database}=fixture();
    await engine.command(context,db=>db.insert('orders',[order('without-validation')]));
