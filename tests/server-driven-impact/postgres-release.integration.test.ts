...
describe.skipIf(!enabled)('PostgreSQL release contract',()=>{
  ...
  const definition=(text:string,parameters:string[]=[],cache?:'no-store'):PostgresSourceDefinition=>({input:parse,source:{text,parameters,...(cache?{cache}:{})}});
  async function install(definitions:Record<string,PostgresSourceDefinition>){
    ...
    return {artifact,engine:createImpact({adapter:adapter(),resources:artifact.resources,queries:artifact.queries})};
  }
        ...
        if(key==='reserve')return async()=>{
          ...
          return new Proxy(session,{
            get(reserved,reservedKey){
              ...
              if(reservedKey==='unsafe')return (text:string,...args:unknown[])=>{statements.push(text);return member.call(reserved,text,...args);};
  ...
  it('executes time, randomness and dynamic SQL only through the no-store API, including composed endpoints',async()=>{
    const {artifact,engine}=await install({clock:definition('select current_timestamp as value'),random:definition('select random() as value'),dynamic:definition(`select "${schema}".dynamic_read() as value`),nested:definition(`select "${schema}".timed_read() as value`)});
  ...
  it('preserves each driver native types, duplicate rows and SQLSTATE without normalizing IDs',async()=>{
    ...
    const {engine}=await install({types:definition(text,[],'no-store'),error:definition('select 1/0',[],'no-store')});
    const native=[...await database.unsafe(text)];
    const actual=await engine.queryUncached('types',{}, {scope:'a'});
    expect(actual.data).toEqual(native); expect(native).toHaveLength(2);
  ...
  it('compares native and SDI results across joins, anti joins, sets, recursion, windows and page boundaries',async()=>{
    ...
    const defs=Object.fromEntries(texts.map((text,index)=>['q'+index,definition(text,text.includes('$1')?['id']:[])]));
    ...
    for(let index=0;index<texts.length;index++){
      ...
      if(JSON.stringify(before[index])!==JSON.stringify(after[index]))expect(changed.impact.targets.some(target=>target.endpoint==='q'+index && matchesInputSelector({id:'one'},target.selector))).toBe(true);
    }
  });

  it('preserves every changed resource when one transaction exceeds the fact budget',async()=>{
    await admin.unsafe(`delete from "${schema}".a where id like 'overflow-a-%';delete from "${schema}".b where id='overflow-b'`);
  ...
  it('finds shadowed CTE reads inside SQL functions and rejects hidden time in views',async()=>{
    const catalog=createPostgresCatalogResolver(admin,resources,{parserVersion:version,searchPath:[schema,'public']});
    expect((await compilePostgresQuery({text:`select "${schema}".shadow_read()`},resources,version,{catalog})).reads.map(read=>read.resource)).toContain('a');
    await admin.unsafe(`create view "${schema}".timed_view as select current_timestamp as now,id from "${schema}".a`);
    await expect(compilePostgresQuery({text:`select * from "${schema}".timed_view`},resources,version,{catalog:createPostgresCatalogResolver(admin,resources,{parserVersion:version})})).rejects.toThrow('POSTGRES_QUERY_REQUIRES_FRESHNESS_POLICY');
    await expect(compilePostgresQuery({text:`select "${schema}".atomic_timed_read()`},resources,version,{catalog})).rejects.toThrow('POSTGRES_QUERY_REQUIRES_FRESHNESS_POLICY');
  });
  ...
  it('recompiles changed views atomically, explicitly detects old artifacts and observes the new dependency',async()=>{
    ...
    const next=await migratePostgresQueries(admin,resources,definitions,{...options(),change:async tx=>{await tx.unsafe(`create or replace view "${schema}".routed as select * from "${schema}".b`);}});
  ...
  it('detects function-only and permission-only drift when explicitly validated',async()=>{
    ...
    await admin.unsafe(`create or replace function "${schema}".shadow_read() returns bigint language sql stable as $$select count(*) from "${schema}".b$$`);
    ...
    try { await expect(second.engine.validate()).rejects.toThrow('POSTGRES_ARTIFACT_DRIFT'); }
    finally { await admin.unsafe(`grant select on "${schema}".a to routine_runtime`); }
  });
  ...
  it('validates against the post-migration catalog snapshot after waiting for its lock',async()=>{
    ...
    await active;
    let settled=false;
    const waiting=engine.validate().then(()=>{settled=true;return undefined;},error=>{settled=true;return error;});
    try { await new Promise(resolve=>setTimeout(resolve,25));expect(settled).toBe(false); }
  ...
  it('routes custom type semantics to native no-store execution and rejects transaction pooling at activation',async()=>{
    await admin.unsafe(`create type "${schema}".mood as enum('ok','bad');create function "${schema}".custom_type_read(value "${schema}".mood) returns "${schema}".mood language sql immutable as $$select $1$$`);
    const {engine,artifact}=await install({custom:definition(`select 'ok'::"${schema}".mood as value`),implicit:definition(`select "${schema}".custom_type_read('ok') as value from "${schema}".a`)});
  ...
  it('observes numeric scale and raw JSON text changes and routes MVCC columns to no-store',async()=>{
    ...
    const artifact=await compilePostgresArtifacts(admin,typed,{text:definition(`select n::text,j::text from "${schema}".typed where id=$1`,['id']),mvcc:definition(`select xmin::text from "${schema}".typed`)},options());
