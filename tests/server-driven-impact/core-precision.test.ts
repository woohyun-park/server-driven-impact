...
import { byteLength, createImpact, LIMITS, matchesInputSelector, WriteSet, type CommandAdapter, type ImpactManifest, type WriteFact } from '@server-driven-impact/core';

const resources = {
  orders: {scopeColumn:'tenant',columns:['tenant','id','customer','value']},
  notes: {scopeColumn:'tenant',columns:['tenant','id','customer','value']},
};
const manifest: ImpactManifest = {protocolVersion:1,reads:{
  orders:[{resource:'orders',columns:'*',bindings:[{column:'customer',input:'customer'},{column:'id',input:'id'}]}],
...
function fact(resource: 'orders'|'notes', id: string, customer = 'customer-a', scope = 'tenant-a'): WriteFact {
  return {resource,operation:'insert',before:{kind:'absent'},after:{kind:'known',scope,fields:{id,customer}},changedColumns:null};
}
const selector = (writes: WriteFact[], endpoint: string) => engine.calculate(writes,'tenant-a').targets.find(target => target.endpoint === endpoint)?.selector;
...
describe('resource-local WriteSet precision under hard limits', () => {
  ...
  it('does not permanently summarize a small resource arriving at an already full buffer', () => {
    ...
    expect(selector(writes.snapshot(),'notes')).toEqual({kind:'inputs',values:[{id:'note-1'},{id:'note-2'}]});
  });
  it('preserves old and new customer ranges across a summarized update batch', () => {
    const writes = new WriteSet();
    writes.add(Array.from({length:LIMITS.facts+1},(_,i) => ({...fact('orders',String(i),'new'),operation:'update',before:fact('orders',String(i),'old').after,changedColumns:['customer']})));
    expect(selector(writes.snapshot(),'orders')).toEqual({kind:'inputs',values:[{customer:'new'},{customer:'old'}]});
  });
  it('preserves UPDATE column pruning and unions later changed columns', () => {
    const valueEngine = createImpact({resources,manifest:{protocolVersion:1,reads:{values:[{resource:'orders',columns:['value'],bindings:[]}]}}});
    const writes = new WriteSet();
    writes.add(Array.from({length:LIMITS.facts+1},(_,i) => ({...fact('orders',String(i)),operation:'update',changedColumns:['customer']})));
  ...
  it('unknown and mixed scopes never become falsely known after later facts', () => {
    const writes = new WriteSet();
    writes.add(Array.from({length:LIMITS.facts+1},(_,i) => fact('orders',String(i),'customer-a',i === 0 ? 'tenant-b' : 'tenant-a')));
  ...
  it('bounds many resources and merges child summaries without losing effects', () => {
    ...
    writes.add([fact('notes','note-1')]); writes.merge(child);
    expect(selector(writes.snapshot(),'orders')).toEqual({kind:'inputs',values:[{customer:'customer-a'}]});
...
describe('endpoint-local impact precision', () => {
  it('retains independent detail when another endpoint exceeds the byte budget', () => {
    const result = engine.explain([fact('notes','note-1'),fact('orders','x'.repeat(LIMITS.impactBytes))],'tenant-a');
    expect(result.impact.targets.find(target => target.endpoint === 'notes')?.selector).toEqual({kind:'inputs',values:[{id:'note-1'}]});
    ...
    expect(result.decisions.some(decision => decision.endpoint === 'orders' && decision.reason === 'byte-limit')).toBe(true);
  });
  it('deduplicates exact conjunction subsets in either encounter order', () => {
    ...
    for (const writes of [[fact('orders','1'),broad],[broad,fact('orders','1')]]) {
  ...
  it('reclaims encoded selector bytes when a partial conjunction subsumes detail', () => {
    ...
    expect(result.targets.find(target => target.endpoint === 'orders')?.selector).toEqual({kind:'inputs',values:[{customer:'customer-a'}]});
  ...
  it('retains a shared customer when selector alternatives exceed their budget', () => {
    ...
    for (let i=0;i<writes.length;i++) expect(matchesInputSelector({customer:'customer-a',id:String(i)},actual)).toBe(true);
...
describe('core command final observer lifecycle', () => {
  it('calculates after commit-time effects are drained into the original WriteSet', async () => {
    const adapter: CommandAdapter<object> = {async command(_scope,work) {
      ...
      writes.add([fact('orders','deferred')]); writes.close();
      return result;
    }};
    const result = await engine.command(adapter,{scope:'tenant-a'},async () => 'committed');
    expect(result.data).toBe('committed');
    expect(result.impact.targets[0].selector).toEqual({kind:'inputs',values:[{customer:'customer-a',id:'deferred'}]});
  });
  it('never returns a successful response after commit or drain rejection', async () => {
    for (const failure of ['COMMIT_FAILED','COMMITTED_OBSERVATION_FAILED']) {
      const adapter: CommandAdapter<object> = {async command(_scope,work) {
        const writes = new WriteSet(); await work({},writes); writes.add([fact('orders','1')]); throw new Error(failure);
      }};
