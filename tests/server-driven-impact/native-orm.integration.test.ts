...
const adminUrl=process.env.SDI_POSTGRES_ADMIN_URL, runtimeUrl=process.env.SDI_POSTGRES_RUNTIME_URL;
...
describe.skipIf(!enabled)('native/Drizzle mutation to frontend conformance', () => {
  ...
  const domain=ordersDomain(schema), repository=orderRepository(schema);
  ...
  const setup=async(tx:{unsafe: (text:string,values?:never[])=>PromiseLike<unknown>},scope:unknown)=>{await tx.unsafe("select set_config('sdi.tenant',$1,true)",[String(scope)] as never[]);};
  ...
  beforeAll(async()=>{
    ...
    await admin.unsafe(generateObserverMigration(domain.resources,compileManifest(domain.queries,domain.resources),{runtimeRole:'routine_runtime'}));
    await orm.validate();
  });
  afterAll(async()=>{await pool.end();await database.end();await admin.unsafe(`drop schema if exists "${schema}" cascade`);await admin.end();});
  it('preserves pg rowMode, codecs, rowCount and rejects transaction control',async()=>{
    const result=await native.command({scope:'a'},async tx=>{
      const decoded=await tx.query({text:'select $1::int as n, $2::jsonb as payload',rowMode:'array'},[7,{ok:true}]);
      expect(decoded.rows).toEqual([[7,{ok:true}]]);
      const date=await tx.query({text:"select '2026-09-10'::date as day",types:{getTypeParser:()=>(v:string)=>'parsed:'+v}});
      expect(date.rows[0].day).toBe('parsed:2026-09-10');
      return tx.query(`insert into "${schema}".orders values($1,$2,$3,$4,$5,$6) returning *`,['native','a','A','ready',1,null]);
    });
    expect(result.data.rowCount).toBe(1); expect(result.data.rows[0].id).toBe('native');
  ...
  it('keeps postgres.js tagged queries lazy and rejects execution after command end',async()=>{
    ...
    const result=await tagged.command({scope:'a'},async tx=>{
      ...
      expect(rows.count).toBe(1); expect(rows[0].n).toBe(7);
      return tx.savepoint(async child=>(await child`select ${8}::int as n`)[0].n);
    });
    expect(result.data).toBe(8); expect(result.impact.targets).toEqual([]);
    await expect(late.execute()).rejects.toThrow('WRITE_CONTEXT_CLOSED');
    await expect(tagged.command({scope:'a'},async tx=>{void tx`select pg_sleep(0.02)`.execute();})).rejects.toThrow('UNAWAITED_DATABASE_OPERATION');
  });
  it('returns precise frontend selectors from repository functions, JOIN and savepoint rollback',async()=>{
    await orm.command({scope:'a'},async tx=>{
      await tx.insert(repository.orders).values({id:'orm',tenant_id:'a',customer_id:'A',status:'ready',priority:2});
    ...
    const result=await orm.command({scope:'a'},async tx=>{
      escaped=tx; late=tx.execute(dsql`select 1`);
    ...
    const cached=[{endpoint:'orders.detail',input:{id:'orm'}},{endpoint:'orders.list',input:{customer:'A'}},{endpoint:'orders.list',input:{customer:'B'}},{endpoint:'orders.list',input:{customer:'C'}},{endpoint:'orders.list',input:{customer:'cancelled'}}];
    expect(affectedQueries(response.impact,cached)).toEqual(cached.slice(0,3));
    expect(response.data.customer_id).toBe('B'); expect(Object.keys(response)).toEqual(['data','impact']);
    await expect(late).rejects.toThrow();
    await expect(escaped.select().from(repository.orders)).rejects.toThrow();
    const deleted=await orm.command({scope:'a'},tx=>tx.delete(repository.orders).where(eq(repository.orders.id,'orm')));
  ...
  it('preserves RLS and SQL error codes through ORM execution',async()=>{
    await expect(orm.command({scope:'b'},tx=>tx.insert(repository.orders).values({id:'forbidden',tenant_id:'a',status:'ready',priority:0}))).rejects.toMatchObject({cause:{code:'42501'}});
  });
});
