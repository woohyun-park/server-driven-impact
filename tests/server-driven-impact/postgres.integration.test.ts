...
import {
  ...
  postgresAdapter,
  resolvePostgresResources,
  Sql,
  sql,
  type PostgresCommandDb as TrackedDb,
...
import {
  ...
  observerInternals,
  observerLayout,
} from '../../packages/sdi-postgres/src/postgres/observer.js';
...
if (enabled) {
  ...
  if (!['127.0.0.1', 'localhost', '::1'].includes(url.hostname) || (!matrix && url.port !== '54332'))
...
describe.skipIf(!enabled)('orders domain / real PostgreSQL conformance', () => {
  ...
  const pgPool =
    enabled && process.env.SDI_POSTGRES_DRIVER === 'pg'
      ? new Pool({ connectionString: runtimeUrl, max: 4 })
      : undefined;
  const db = enabled
    ? pgPool
      ? ({ ...pgDatabase(pgPool), end: () => pgPool.end() } as unknown as postgres.Sql)
      : postgres(runtimeUrl!, { max: 4, prepare: false })
    : undefined!;
  let serverMajor = 0;
  const setup = async (
    tx: Parameters<NonNullable<Parameters<typeof postgresAdapter>[0]['setup']>>[0],
    scope: unknown,
  ) => {
    await tx.unsafe("select set_config('sdi.tenant',$1,true)", [String(scope)]);
  };
  ...
  const driverRows = (result: unknown) =>
  ...
  const insert = async (tx: TrackedDb, resource: string, rows: Record<string, unknown>[]) => {
    ...
    const tuples = rows
      .map((_, row) => `(${names.map((__, column) => `$${row * names.length + column + 1}`).join(',')})`)
      .join(',');
    return tx.execute(
      new Sql(
        `insert into "${schema}"."${table}"(${names.map(name => `"${name}"`).join(',')}) values ${tuples} returning *`,
        values,
      ),
    );
  };
  ...
  beforeAll(async () => {
    ...
    if (expected !== undefined && serverMajor !== expected)
      throw new Error(`POSTGRES_MAJOR_MISMATCH:${serverMajor}:${expected}`);
  ...
  it('rejects narrow selectors on nondeterministic collations during validation', async () => {
    await admin.unsafe(`create table "${schema}".exact_strings(id text primary key,value varchar not null)`);
    const exact = {
    ...
    const collated = {
      value: { schema, table: 'collated', idColumn: 'id', scopeColumn: null, columns: ['id', 'value'] },
    };
    await expect(
  ...
  it('empty→insert, customer move, joins, aggregates, cascade and tenant isolation', async () => {
    ...
    const inserted = await run('a', tx =>
      insert(tx, 'orders', [
        { id: 'one', tenant_id: 'a', customer_id: 'first', status: 'ready', priority: 1, note: null },
      ]),
    );
    ...
    await expect(
      run('b', tx =>
        insert(tx, 'orders', [
          { id: 'forbidden', tenant_id: 'a', customer_id: null, status: 'ready', priority: 0, note: null },
        ]),
      ),
    ).rejects.toMatchObject({ code: '42501' });
    const moved = await run('a', tx =>
      tx.execute(sql`update ${identifier(schema)}.orders set customer_id=${'second'} where id=${'one'}`),
    );
    for (const customer of ['first', 'second']) expect(includes(moved.impact, 'orders.list', { customer })).toBe(true);
    const item = await run('a', tx =>
      insert(tx, 'items', [{ id: 'item', tenant_id: 'a', order_id: 'one', amount: 42 }]),
    );
    expect(includes(item.impact, 'orders.detail', { id: 'one' })).toBe(true);
    expect(includes(item.impact, 'orders.total', { id: 'one' })).toBe(true);
    ...
    expect(await read('orders.total', { id: 'one' })).toBe(0);
  });
  it('rollback/savepoint, no-op, commit failure and closed contexts', async () => {
    ...
    await expect(
      run('a', async tx => {
        captured = tx;
        await insert(tx, 'orders', [
          { id: 'rolled', tenant_id: 'a', customer_id: 'x', status: 'ready', priority: 0, note: null },
        ]);
    ...
    const result = await run('a', async tx => {
      await expect(
        tx.savepoint(async child => {
          await insert(child, 'orders', [
            { id: 'child', tenant_id: 'a', customer_id: 'child', status: 'ready', priority: 0, note: null },
          ]);
          throw new Error('child rollback');
        }),
      ).rejects.toThrow('child rollback');
      await tx.savepoint(child =>
        insert(child, 'orders', [
          { id: 'kept', tenant_id: 'a', customer_id: 'kept', status: 'ready', priority: 0, note: null },
        ]),
      );
    });
    ...
    await expect(
      run('a', tx =>
        insert(tx, 'orders', [
          { id: 'bad1', tenant_id: 'a', customer_id: null, status: 'ready', priority: 0, note: 'duplicate' },
          { id: 'bad2', tenant_id: 'a', customer_id: null, status: 'ready', priority: 0, note: 'duplicate' },
        ]),
      ),
  ...
  it('serializes artifact migration against an active SDI transaction', async () => {
    ...
    const command = engine.command({ scope: 'a' }, async () => {
      entered();
      await gate;
      return 'held';
    });
    await active;
    let migrated = false;
    const migration = migratePostgresArtifacts(admin, resources, compileManifest(queries, resources), {
  ...
  it('fixed-seed result-change implies impact, across WHERE/ORDER and scalar predicates', async () => {
    ...
    const inputs = [
      ['orders.list', { customer: 'odd' }],
      ['orders.list', { customer: 'even' }],
      ['orders.ready', {}],
      ...Array.from({ length: 8 }, (_, i) => ['orders.detail', { id: 'seed' + i }]),
    ] as [keyof typeof queries, Record<string, unknown>][];
    for (let i = 0; i < 32; i++) {
      const before = await Promise.all(inputs.map(([endpoint, input]) => read(endpoint, input)));
      const id = 'seed' + (rand() % 8);
      const result = await run('a', tx =>
        tx.execute(
          sql`update ${identifier(schema)}.orders set status=${rand() % 2 ? 'ready' : 'draft'},priority=${rand() % 20},customer_id=${rand() % 2 ? 'odd' : 'even'} where id=${id}`,
        ),
      );
      const after = await Promise.all(inputs.map(([endpoint, input]) => read(endpoint, input)));
      for (let j = 0; j < inputs.length; j++)
        if (canonical(before[j]) !== canonical(after[j]))
          expect(includes(result.impact, ...inputs[j]), `${i} ${inputs[j][0]}`).toBe(true);
    }
  });
  ...
  it('executes native PostgreSQL DML unchanged on the observed transaction', async () => {
    ...
    const inserted = await engine.command({ scope: 'a' }, db =>
      db.execute(sql`
    ...
    if (serverMajor >= 15) {
      const merged = await engine.command({ scope: 'a' }, db =>
        db.execute(sql`
  ...
  it('observes composite-key and keyless resources without inverse reads', async () => {
    ...
    await admin.unsafe(
      generateObserverMigration(extraResources, compileManifest(extraQueries, extraResources), {
        runtimeRole: 'routine_runtime',
      }),
    );
    const extra = createImpact({
      adapter: postgresAdapter({ database: db }),
      resources: extraResources,
      queries: extraQueries,
    });
    await extra.validate();
    const inserted = await extra.command({ scope: 'a' }, tx =>
      tx.execute(sql`insert into ${identifier(schema)}.composite_runtime(a,b,value) values(${1},${2},${'x'})`),
    );
    expect(includes(inserted.impact, 'composite.byA', { a: 1 })).toBe(true);
    const keyless = await extra.command({ scope: 'a' }, tx =>
      tx.execute(sql`insert into ${identifier(schema)}.keyless_runtime(a,value) values(${7},${'x'})`),
    );
    expect(includes(keyless.impact, 'keyless.byA', { a: 7 })).toBe(true);
    const changed = await extra.command({ scope: 'a' }, tx =>
      tx.execute(sql`update ${identifier(schema)}.keyless_runtime set value=${'y'} where a=${7}`),
    );
    expect(includes(changed.impact, 'keyless.byA', { a: 7 })).toBe(true);
    const called = await extra.command({ scope: 'a' }, tx =>
      tx.execute(sql`select ${identifier(schema)}.add_keyless(${9},${'rpc'}) as value`),
    );
  ...
  it('executes an automatically compiled native SQL query and narrows its input impact', async () => {
    ...
    const nativeResources = {
      native: { schema, table: 'NativeRows', idColumn: 'Key', scopeColumn: null, columns: ['Key', 'Value', 'Payload'] },
    } as const;
    const plan = await compilePostgresQuery(
      { text: `select "Key","Value","Payload" from "${schema}"."NativeRows" where "Key"=$1`, parameters: ['id'] },
      nativeResources,
      17,
    );
    const nativeQueries = defineQueries({
      'native.byId': { input: { parse: (value: unknown) => value as Record<string, unknown> }, plan },
    });
    await admin.unsafe(
      generateObserverMigration(nativeResources, compileManifest(nativeQueries, nativeResources), {
        runtimeRole: 'routine_runtime',
      }),
    );
    const native = createImpact({
      adapter: postgresAdapter({ database: db }),
    ...
    const result = await native.command({ scope: 'a' }, db =>
      db.execute(
        sql`insert into ${table}(${identifier('Key')},${identifier('Value')},${identifier('Payload')}) values(${'one'},${'hello'},${'{"a":1}'}::json)`,
      ),
    );
    expect(includes(result.impact, 'native.byId', { id: 'one' })).toBe(true);
    expect(includes(result.impact, 'native.byId', { id: 'two' })).toBe(false);
    const updated = await native.command({ scope: 'a' }, db =>
      db.execute(
        sql`update ${table} set ${identifier('Payload')}=${'{"a":2}'}::json where ${identifier('Key')}=${'one'}`,
      ),
    );
    expect(includes(updated.impact, 'native.byId', { id: 'one' })).toBe(true);
    expect(await native.query('native.byId', { id: 'one' }, { scope: 'a' })).toEqual([
      { Key: 'one', Value: 'hello', Payload: pgPool ? { a: 2 } : '{"a":2}' },
    ]);
  });
  it('catalog accepts composite and keyless tables while retaining unsupported table checks', async () => {
    ...
    ).rejects.toThrow('UNSUPPORTED_TABLE');
    await admin.unsafe(
      `create table "${schema}".parent(id int primary key);create table "${schema}".child() inherits("${schema}".parent)`,
    );
    await expect(
      validateCatalog(admin, {
        test: { schema, table: 'parent', idColumn: 'id', scopeColumn: null, columns: ['id'], selectorColumns: ['id'] },
      }),
    ).rejects.toThrow('UNSUPPORTED_TABLE');
    ...
    await expect(
      validateCatalog(admin, {
        test: { schema, table: 'secured', idColumn: 'id', scopeColumn: null, columns: ['id', 'uid'] },
      }),
  ...
  it('expands nested view and RLS helper dependencies through the catalog', async () => {
    ...
    const catalog = createPostgresCatalogResolver(admin, securedResources, {
      searchPath: [schema, 'public'],
      parserVersion: serverMajor as 14 | 15 | 16 | 17 | 18,
    });
    const plan = await compilePostgresQuery(
      { text: `select * from "${schema}".nested_secured_view where id=$1`, parameters: ['id'] },
      securedResources,
      serverMajor as 14 | 15 | 16 | 17 | 18,
      { catalog },
    );
    ...
    const securedQueries = defineQueries({
      'secured.byId': { input: { parse: (value: unknown) => value as Record<string, unknown> }, plan },
    });
    const securedManifest = compileManifest(securedQueries, securedResources);
    await admin.unsafe(
      generateObserverMigration(securedResources, securedManifest, { runtimeRole: 'routine_runtime' }),
    );
    const securedEngine = createImpact({
      adapter: postgresAdapter({ database: db }),
      resources: securedResources,
      queries: securedQueries,
    });
    ...
    expect(await securedEngine.query('secured.byId', { id: 'visible' }, { scope: 'a' })).toEqual([
      { id: 'visible', tenant_id: 'a', value: 'value' },
    ]);
  });
  it('does not miss self-join, untracked function reads, or non-row freshness dependencies', async () => {
    ...
    const compatibilityResources = {
      users: { schema, table: 'compatibility_users', idColumn: 'id', scopeColumn: null, columns: ['id', 'name'] },
    } as const;
    const catalog = createPostgresCatalogResolver(admin, compatibilityResources, {
      searchPath: [schema, 'public'],
      parserVersion: serverMajor as 14 | 15 | 16 | 17 | 18,
    });
    ...
    const pairQueries = defineQueries({
      pair: { input: { parse: (value: unknown) => value as Record<string, unknown> }, plan: pairPlan },
    });
    await admin.unsafe(
      generateObserverMigration(compatibilityResources, compileManifest(pairQueries, compatibilityResources), {
        runtimeRole: 'routine_runtime',
      }),
    );
    const pair = createImpact({
      adapter: postgresAdapter({ database: db }),
    ...
    const changed = await pair.command({ scope: 'a' }, db =>
      db.execute(
        sql`update ${identifier(schema)}.${identifier('compatibility_users')} set name=${'Changed'} where id=${'a'}`,
      ),
    );
    ...
    await expect(
      compilePostgresQuery(
        { text: `select lower(name) from "${schema}".compatibility_users` },
        compatibilityResources,
        serverMajor as 14 | 15 | 16 | 17 | 18,
        { catalog },
      ),
    ...
    await expect(
      compilePostgresQuery(
        { text: `select "${schema}".hidden_reader()` },
        compatibilityResources,
        serverMajor as 14 | 15 | 16 | 17 | 18,
        { catalog },
      ),
    ).rejects.toThrow(`UNTRACKED_QUERY_RELATION:${schema}.compatibility_hidden`);
    const discovering = createPostgresCatalogResolver(admin, compatibilityResources, {
      searchPath: [schema, 'public'],
      parserVersion: serverMajor as 14 | 15 | 16 | 17 | 18,
      discoverUnregisteredRelations: true,
    });
    const hiddenPlan = await compilePostgresQuery(
      { text: `select "${schema}".hidden_reader()` },
      compatibilityResources,
      serverMajor as 14 | 15 | 16 | 17 | 18,
      { catalog: discovering },
    ...
    expect(discovering.resources[hiddenResource]).toEqual({
      schema,
      table: 'compatibility_hidden',
      idColumn: 'id',
      scopeColumn: null,
      columns: ['id'],
    });
    const hiddenQueries = defineQueries({
      hidden: { input: { parse: (value: unknown) => value as Record<string, unknown> }, plan: hiddenPlan },
    });
    await admin.unsafe(
      generateObserverMigration(discovering.resources, compileManifest(hiddenQueries, discovering.resources), {
        runtimeRole: 'routine_runtime',
      }),
    );
    const hidden = createImpact({
      adapter: postgresAdapter({ database: db }),
    ...
    const hiddenChanged = await hidden.command({ scope: 'a' }, db =>
      db.execute(sql`delete from ${identifier(schema)}.${identifier('compatibility_hidden')} where id=${'a'}`),
    );
    expect(await hidden.query('hidden', {}, { scope: 'a' })).not.toEqual(hiddenBefore);
    expect(includes(hiddenChanged.impact, 'hidden', {})).toBe(true);
    await expect(
      compilePostgresQuery(
        { text: `select now(),id from "${schema}".compatibility_users` },
        compatibilityResources,
        serverMajor as 14 | 15 | 16 | 17 | 18,
        { catalog },
      ),
  ...
  it('observes parent and direct-leaf partition and inheritance writes as one logical resource', async () => {
    ...
    await admin.unsafe(
      generateObserverMigration(hierarchyResources, hierarchyManifest, { runtimeRole: 'routine_runtime' }),
    );
    const hierarchy = createImpact({
      adapter: postgresAdapter({ database: db }),
      resources: hierarchyResources,
    ...
    const parentWrite = await hierarchy.command({ scope: 'a' }, db =>
      db.execute(sql`insert into ${events} values(${1},${1},${'parent'})`),
    );
    expect(includes(parentWrite.impact, 'events.byId', { id: 1 })).toBe(true);
    const leafWrite = await hierarchy.command({ scope: 'a' }, db =>
      db.execute(sql`insert into ${early} values(${2},${2},${'leaf'})`),
    );
    expect(includes(leafWrite.impact, 'events.byId', { id: 2 })).toBe(true);
    const moved = await hierarchy.command({ scope: 'a' }, db =>
      db.execute(sql`update ${events} set bucket=${11} where id=${1}`),
    );
    expect(includes(moved.impact, 'events.byId', { id: 1 })).toBe(true);
    const inheritedWrite = await hierarchy.command({ scope: 'a' }, db =>
      db.execute(sql`insert into ${inheritedChild}(id,value,extra) values(${3},${'child'},${'extra'})`),
    );
    ...
    const future = await next.command({ scope: 'a' }, db =>
      db.execute(sql`insert into ${events} values(${4},${21},${'future'})`),
    );
  ...
  it('keeps COPY and cursor protocols inside the guarded physical session', async () => {
    ...
    await expect(
      engine.command({ scope: 'a' }, db =>
        db.copyFrom(sql`copy ${orders}(id,tenant_id,customer_id,status,priority,note) from stdin with (format csv)`, [
    ...
    const copyResources = {
      copy: { schema, table: 'copy_rows', idColumn: 'id', scopeColumn: null, columns: ['id', 'group_id', 'value'] },
    } as const;
    const copyQueries = defineQueries({
      'copy.byGroup': {
        input: { parse: (value: unknown) => value as Record<string, unknown> },
        plan: q.select('copy', { where: [q.eq('group_id', q.input('group'))] }),
      },
    });
    await admin.unsafe(
      generateObserverMigration(copyResources, compileManifest(copyQueries, copyResources), {
        runtimeRole: 'routine_runtime',
      }),
    );
    const copyEngine = createImpact({
      adapter: postgresAdapter({ database: db }),
    ...
    const copied = await copyEngine.command({ scope: 'a' }, db =>
      db.copyFrom(sql`copy ${table}(id,group_id,value) from stdin with (format csv)`, [
        'copy-one,copy,1\n',
        'copy-two,copy,2\n',
      ]),
    );
    ...
    const cursorRows = await copyEngine.command({ scope: 'a' }, async db => {
      ...
      for await (const batch of db.cursor(sql`select id from ${table} where id like ${'copy-%'} order by id`, 1))
        rows.push(...batch);
      return rows;
    });
    ...
    const exported = await copyEngine.command({ scope: 'a' }, async db => {
      ...
      for await (const chunk of db.copyTo(
        sql`copy (select id from ${table} where id like 'copy-%' order by id) to stdout`,
      ))
        chunks.push(chunk);
    ...
    await expect(
      copyEngine.command({ scope: 'a' }, async db => {
  ...
  it('marks a materialized view only after its refresh succeeds', async () => {
    ...
    const materialQueries = defineQueries({
      'snapshot.all': {
        input: { parse: (value: unknown) => value as Record<string, unknown> },
        plan: q.select('snapshot'),
      },
    });
    await admin.unsafe(
      generateObserverMigration(materialResources, compileManifest(materialQueries, materialResources), {
        runtimeRole: 'routine_runtime',
      }),
    );
    const material = createImpact({
      adapter: postgresAdapter({ database: db }),
    ...
    const rolledBack = await material.command({ scope: 'a' }, async db => {
      await expect(
        db.savepoint(async child => {
          await child.refreshMaterializedView('snapshot');
    ...
    await expect(material.command({ scope: 'a' }, db => db.refreshMaterializedView('missing'))).rejects.toThrow(
