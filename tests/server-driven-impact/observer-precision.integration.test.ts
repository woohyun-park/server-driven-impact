...
const definitions=(schema:string)=>{
  const resources:Resources={
    ...
    children:{schema,table:'children',idColumn:'id',scopeColumn:'tenant',columns:['id','tenant','parent_id']},
    audit:{schema,table:'audit',idColumn:'id',scopeColumn:'tenant',columns:['id','tenant','old_customer','new_customer']},
    deferred:{schema,table:'deferred',idColumn:'id',scopeColumn:'tenant',columns:['id','tenant','customer']},
  ...
  const by=(resource:string,column:string,field=column)=>({input,plan:q.select(resource,{where:[q.eq(column,q.input(field))]})});
  const queries={parent:by('parents','customer'),child:by('children','parent_id'),oldAudit:by('audit','old_customer'),newAudit:by('audit','new_customer'),deferred:by('deferred','customer'),largeList:by('large','customer'),largeDetail:by('large','id'),small:by('small','id')};
  return {resources,manifest:compileManifest(queries,resources)};
};
const rows=(result:unknown):Record<string,unknown>[]=>Array.isArray(result)?result:(result as {rows:Record<string,unknown>[]}).rows;
...
describe.skipIf(!enabled)('PostgreSQL collected WriteSet completeness and precision',()=>{
  ...
  it('collects committed direct, cascade, trigger and deferred effects with OLD/NEW and discards rolled-back work',async()=>{
    ...
    await bound.command('a',writes,async tx=>{
      await expect(tx.savepoint(async child=>{
    ...
    expect(facts).toEqual(expect.arrayContaining([
      expect.objectContaining({resource:'parents',before:known('a',{id:'before',tenant:'a',customer:'A'}),after:{kind:'absent'}}),
      expect.objectContaining({resource:'parents',before:{kind:'absent'},after:known('a',{id:'after',tenant:'a',customer:'B'})}),
      expect.objectContaining({resource:'children',before:known('a',{id:'line',tenant:'a',parent_id:'before'}),after:known('a',{id:'line',tenant:'a',parent_id:'after'}),changedColumns:['parent_id']}),
      expect.objectContaining({resource:'audit',operation:'insert',after:known('a',{id:'after',tenant:'a',old_customer:'A',new_customer:'B'})}),
      expect.objectContaining({resource:'deferred',operation:'insert',after:known('a',{id:'after',tenant:'a',customer:'B'})}),
    ]));
    const rollback=new WriteSet();
    await expect(bound.command('a',rollback,async tx=>{
  ...
  it('preserves common batch OLD/NEW scope and bindings without widening a small independent resource',async()=>{
    ...
    expect(facts.find(fact=>fact.resource==='large')).toEqual({resource:'large',operation:'unknown',before:known('a',{tenant:'a',customer:'A'}),after:known('a',{tenant:'a',customer:'B'}),changedColumns:null});
    ...
    expect(impact.targets.find(target=>target.endpoint==='largeList')?.selector).toEqual({kind:'inputs',values:[{customer:'A'},{customer:'B'}]});
    expect(impact.targets.find(target=>target.endpoint==='largeDetail')?.selector).toEqual({kind:'all'});
    expect(impact.targets.find(target=>target.endpoint==='small')?.selector).toEqual({kind:'inputs',values:[{id:'precise'}]});
  ...
  it('widens mixed-scope batches so neither caller loses its actual result change',async()=>{
    const writes=new WriteSet();
    await bound.command('a',writes,tx=>tx.execute(sql`insert into ${identifier(schema)}.large select 'mixed-'||n,case when n%2=0 then 'a' else 'b' end,'shared' from generate_series(1,${LIMITS.facts+25}) n`));
    const facts=writes.snapshot();
    expect(facts).toEqual([{resource:'large',operation:'unknown',before:{kind:'absent'},after:{kind:'unknown'},changedColumns:null}]);
    for(const scope of ['a','b']){
      const actual=await admin.unsafe(`select count(*)::int as n from "${schema}".large where tenant=$1 and customer='shared'`,[scope]);
      expect(actual[0].n).toBeGreaterThan(0);
      const impact=calculateImpact(facts,{resources,manifest,scope});
      expect(impact.targets.some(target=>target.endpoint==='largeList' && matchesInputSelector({customer:'shared'},target.selector))).toBe(true);
    }
  });
});
...
it('SQLite collector overflow is resource-local and savepoint rollback cannot poison retained facts',async()=>{
  ...
  try{
    database.exec('create table large(id text primary key,tenant text,customer text);create table small(id text primary key,tenant text,customer text);');
    const full=definitions('main');
    const resources={large:full.resources.large,small:full.resources.small};
    const manifest=compileManifest({large:{input,plan:q.select('large',{where:[q.eq('customer',q.input('customer'))]})},small:{input,plan:q.select('small',{where:[q.eq('id',q.input('id'))]})}},resources);
    const bound=sqliteAdapter({database})[bindAdapter](resources,manifest);
    const writes=new WriteSet();
    await bound.command('a',writes,async tx=>{
      await expect(tx.savepoint(async child=>{
        await child.execute(`with recursive n(x) as (values(1) union all select x+1 from n where x<${LIMITS.facts+25}) insert into large select 'discard-'||x,'a','discarded' from n`);
        throw new Error('discard');
      })).rejects.toThrow('discard');
      await tx.execute("insert into small values('precise','a','kept')");
      await tx.execute(`with recursive n(x) as (values(1) union all select x+1 from n where x<${LIMITS.facts+25}) insert into large select 'kept-'||x,'a','kept' from n`);
    });
    const facts:WriteFact[]=writes.snapshot();
    expect(facts).toHaveLength(2);
    expect(facts.find(fact=>fact.resource==='large')).toEqual({resource:'large',operation:'unknown',before:{kind:'unknown'},after:{kind:'unknown'},changedColumns:null});
    expect(facts.find(fact=>fact.resource==='small')?.after).toEqual(known('a',{id:'precise',tenant:'a'}));
