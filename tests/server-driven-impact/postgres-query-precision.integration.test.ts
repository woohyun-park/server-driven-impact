...
describe.skipIf(!enabled)('PostgreSQL automatic observed-column precision', () => {
  ...
  const resources: Resources = {
    records: {
      ...
      idColumn: 'id',
      scopeColumn: null,
      columns: ['id', 'status', 'rank', 'label_id', 'note'],
    },
  ...
  it('automatically excludes unrelated updates and includes actual filter, page, join and RLS result changes', async () => {
    ...
    const definition = (text: string, parameters: string[] = []) => ({
      input,
      source: { text, parameters },
    ...
    const artifact = await compilePostgresArtifacts(
      ...
      resources,
      {
        ...
        joined: definition(
          `select r.id,l.label from "${schema}".records r join "${schema}".labels l on l.id=r.label_id where r.status=$1 order by r.id`,
          ['status'],
        ),
    ...
    expect(artifact.manifest.reads.page).toEqual([
      { resource: 'records', columns: ['id', 'rank', 'status'], bindings: [] },
    ]);
    expect(artifact.manifest.reads.count).toEqual([{ resource: 'records', columns: ['status'], bindings: [] }]);
    expect(artifact.manifest.reads.secured).toEqual([
      { resource: 'secured', columns: ['id', 'visible'], bindings: [] },
    ]);
    await admin.unsafe(
    ...
    const read = () =>
      Promise.all([
        ...
        engine.query('page', { status: 'ready' }, context),
        engine.query('count', { status: 'ready' }, context),
        engine.query('joined', { status: 'ready' }, context),
      ]);
    const before = await read();
    const irrelevant = await engine.command(context, tx =>
    ...
    const filtered = await engine.command(context, tx =>
      tx.execute(sql`update ${identifier(schema)}.records set status='done' where id='two'`),
    );
    ...
    const joined = await engine.command(context, tx =>
      tx.execute(sql`update ${identifier(schema)}.labels set label='new'`),
    );
