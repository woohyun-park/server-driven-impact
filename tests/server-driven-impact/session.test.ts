...
it('closes all open iterators and surfaces protocol cleanup failure',async()=>{
  const close=vi.fn(async()=>{throw new Error('protocol lost');});
  const guarded=guardDatabase({cursor:()=>({[Symbol.asyncIterator](){return {next:async()=>({done:false,value:1}),return:close};}})});
...
it('never returns a broken postgres.js connection to the pool when backend termination is forbidden',async()=>{
  const release=vi.fn();
  const session={unsafe:vi.fn(async()=>{throw Object.assign(new Error('denied'),{code:'42501'});}),release} as unknown as ReservedSession;
