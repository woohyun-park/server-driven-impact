...
if (pgEnabled && !['127.0.0.1', 'localhost', '::1'].includes(new URL(adminUrl!).hostname))
...
async function insert(
  ...
  if (dialect === 'sqlite') {
    ...
    const result = await (db as SqliteCommandDb).execute(
      `insert into ${relation(resource)}(${names.join(',')}) values ${tuples} returning *`,
      values,
    );
  ...
  const tuples = rows
    ...
    .join(',');
  const result = await (db as PostgresCommandDb).execute(
    new Sql(`insert into ${table.text}(${columns}) values ${tuples} returning *`, parameters),
  );
  return { count: result.length, rows: returnRows ? result : [] };
}
async function update(
  ...
  const setEntries = Object.entries(set),
    ...
    values = [...setEntries, ...whereEntries].map(([, value]) => value as null | string | number);
  const marker = (index: number) => (dialect === 'sqlite' ? '?' : `$${index}`);
  const assignments = setEntries.map(([name], index) => `"${name}"=${marker(index + 1)}`).join(',');
  const predicate = whereEntries.length
    ? whereEntries
        .map(([name, value], index) =>
          value === null ? `"${name}" is null` : `"${name}"=${marker(setEntries.length + index + 1)}`,
        )
        .join(' and ')
    : 'true';
  const bindings = values.filter(
    (_, index) => index < setEntries.length || whereEntries[index - setEntries.length]?.[1] !== null,
  );
...
async function remove(
  ...
  const entries = Object.entries(where),
    table = dialect === 'sqlite' ? relation(resource) : `"${schema}"."${relation(resource)}"`;
  ...
  const rows =
    dialect === 'sqlite'
      ? await (db as SqliteCommandDb).execute(statement, values)
      : await (db as PostgresCommandDb).execute(new Sql(statement, values));
...
async function selectPriority(dialect: 'sqlite' | 'postgres', db: TestDb, schema: string) {
  ...
  return (db as PostgresCommandDb).execute(
    sql`select id from ${identifier(schema)}.${identifier('orders')} where priority>=${1}`,
  );
}
for (const dialect of ['sqlite', 'postgres'] as const)
  describe.skipIf(dialect === 'postgres' && !pgEnabled)(`${dialect}: common API conformance`, () => {
      ...
      if (dialect === 'sqlite') {
        ...
        await admin.unsafe(
          generateObserverMigration(
            ...
            compileManifest(definitions.queries, definitions.resources),
            { runtimeRole: 'routine_runtime' },
          ),
        );
        reset = async () => {
          await admin.unsafe(`truncate "${schema}".order_items, "${schema}".orders`);
        };
      ...
      expect(
        (
          await engine.command(context, db =>
            update(dialect, schema, db, 'orders', { customer_id: 'second' }, { customer_id: null }),
          )
        ).data.count,
      ).toBe(1);
    ...
    it('transaction/savepoint rollback and closed handles cannot publish writes', async () => {
      ...
      await expect(
        engine.command(context, async db => {
          captured = db;
          await insert(dialect, schema, db, 'orders', [order('rolled')]);
      ...
      await expect(
        dialect === 'sqlite'
          ? (captured as SqliteCommandDb).execute('select 1')
          : (captured as PostgresCommandDb).execute(sql`select 1`),
      ).rejects.toThrow('WRITE_CONTEXT_CLOSED');
      const result = await engine.command(context, async db => {
        await expect(
          db.savepoint(async child => {
            await insert(dialect, schema, child, 'orders', [order('child')]);
    ...
    it('fixed-seed changes in actual query results always have matching impact metadata', async () => {
      ...
      const inputs: [keyof typeof definitions.queries, Record<string, unknown>][] = [
        ...
        ['orders.ready', {}],
        ...Array.from({ length: 8 }, (_, i): [keyof typeof definitions.queries, Record<string, unknown>] => [
          'orders.detail',
          { id: 'seed' + i },
        ]),
      ];
      ...
      for (let i = 0; i < 20; i++) {
        ...
        const changed = await engine.command(context, db =>
          update(
            ...
            'orders',
            {
              status: random() % 2 ? 'ready' : 'draft',
              priority: random() % 20,
              customer_id: random() % 2 ? 'odd' : 'even',
            },
            { id: 'seed' + (random() % 8) },
          ),
        );
