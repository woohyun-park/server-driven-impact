...
const contract = defineCacheContract({
  ...
  queries: [
    {
      operationId: 'getA', endpoint: 'A.detail', kind: 'query',
      input: {id: {type: 'number', required: true, coerce: true}},
      key: {prefix: ['A'], path: ['id']}, fallback: ['A'],
    },
    {
      operationId: 'listA', endpoint: 'A.list', kind: 'query',
...
describe('server-owned cache contracts', () => {
  ...
  it('widens instead of dropping selectors that the key contract cannot prove', () => {
    const result = compileCacheInvalidations(contract, {
      protocolVersion: 1,
  ...
  it('widens when missing-field matches cannot be represented by a partial key', () => {
    const unanchored = defineCacheContract({
      id: 'unanchored', version: 1, queries: [{
        operationId: 'listA', endpoint: 'A.list', kind: 'query',
        input: {id: {type: 'number'}, page: {type: 'number'}},
        key: {prefix: ['A', 'list'], params: ['id', 'page'], omitEmptyParams: true}, fallback: ['A', 'list'],
      }],
    });
    expect(compileCacheInvalidations(unanchored, {
      protocolVersion: 1,
      targets: [{endpoint: 'A.list', scope: 'caller', selector: {kind: 'inputs', values: [{id: 1}]}}],
    }, 'tenant-a').invalidations).toEqual([{queryKey: ['A', 'list'], exact: false}]);
  });
  ...
  it('rejects potentially colliding detail and list templates', () => {
    expect(() => defineCacheContract({
      id: 'collision', version: 1, queries: [
        {operationId: 'detail', endpoint: 'A.detail', kind: 'query', input: {id: {type: 'string', required: true}}, key: {prefix: ['A'], path: ['id']}, fallback: ['A']},
        {operationId: 'list', endpoint: 'A.list', kind: 'query', input: {}, key: {prefix: ['A', 'list']}, fallback: ['A', 'list']},
      ],
    })).toThrow('CACHE_KEY_COLLISION');
    expect(() => defineCacheContract({
      id: 'separated', version: 1, queries: [
        {operationId: 'detail', endpoint: 'A.detail', kind: 'query', input: {id: {type: 'string', required: true, exclude: ['list']}}, key: {prefix: ['A'], path: ['id']}, fallback: ['A']},
        {operationId: 'list', endpoint: 'A.list', kind: 'query', input: {}, key: {prefix: ['A', 'list']}, fallback: ['A', 'list']},
      ],
    })).not.toThrow();
  });
  ...
  it('widens the whole endpoint instead of truncating an over-budget response', () => {
    const result = compileCacheInvalidations(contract, {
      protocolVersion: 1,
      targets: [{endpoint: 'A.list', scope: 'caller', selector: {kind: 'inputs', values: Array.from({length: 600}, (_, id) => ({id}))}}],
    }, 'tenant-a');
    expect(result.invalidations).toEqual([{queryKey: ['A', 'list'], exact: false}]);
  });
  ...
  it('integrates with the runtime and rejects unsupported versions before mutation', async () => {
    ...
    const resources: Resources = {A: {schema: 'main', table: 'A', idColumn: 'id', scopeColumn: 'tenant', columns: ['id', 'tenant']}};
    const queries = defineQueries({
      'A.detail': {input: {parse(value: unknown): Input { return value as Input; }}, plan: q.select('A', {where: [q.eq('id', q.input('id'))]})},
      'A.list': {input: {parse(value: unknown): Input { return value as Input; }}, plan: q.select('A')},
    });
    const engine = createImpact({resources, queries, adapter: sqliteAdapter({database}), cacheContracts: [contract]});
    let invoked = false;
    await expect(engine.command({scope: 'tenant-a'}, async () => { invoked = true; }, {cacheContract: {id: 'company-api', version: 2}})).rejects.toThrow('UNSUPPORTED_CACHE_CONTRACT');
    expect(invoked).toBe(false);
    const result = await engine.command({scope: 'tenant-a'}, db => db.execute('insert into A(id,tenant) values(?,?)', [1, 'tenant-a']), {cacheContract: {id: 'company-api', version: 1}});
...
describe('TanStack Query invalidation executor', () => {
  it('invalidates the exact/partial union once and leaves unrelated keys fresh', async () => {
    ...
    const keys = [
      ['A', 1], ['A', 2], ['A', 'list'],
      ['A', 'list', {id: 1}], ['A', 'list', {id: 1, page: 2}], ['A', 'list', {id: 2}],
    ] as const;
    ...
    await applyCacheInvalidations(client, payload, {
      contract: {id: 'company-api', version: 1}, scope: 'tenant-a', refetchType: 'active',
    });
  ...
  it('rejects stale scope and contract responses before touching the cache', async () => {
    ...
    await expect(applyCacheInvalidations(client, payload, {
      contract: {id: 'company-api', version: 1}, scope: 'new-tenant',
  ...
  it('rejects malformed network keys before touching the cache', async () => {
    ...
    const invalidate = vi.spyOn(client, 'invalidateQueries');
    const malformed = {
      protocolVersion: 1, contractId: 'company-api', contractVersion: 1, scope: 'tenant-a',
      invalidations: [{queryKey: ['A', Number.NaN], exact: false}],
    } as unknown as CacheInvalidationSet;
    await expect(applyCacheInvalidations(client, malformed, {
      contract: {id: 'company-api', version: 1}, scope: 'tenant-a',
