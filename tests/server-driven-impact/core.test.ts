...
const resources: Resources = {
  records: {
    ...
    idColumn: 'id',
    scopeColumn: 'tenant',
    columns: ['id', 'tenant', 'filter', 'value', 'sort'],
    selectorColumns: ['id', 'filter'],
  },
};
...
const known = (filter: string | number | boolean | null, scope = 'a') => ({
  kind: 'known' as const,
  scope,
  fields: { filter },
});
const fact = (filter: string | number | boolean | null): WriteFact => ({
  resource: 'records',
  operation: 'insert',
  before: { kind: 'absent' },
  after: known(filter),
    ...
    rowsToFacts([
      {
        resource: 'records',
        operation: 'update',
        before_state: { kind: 'known', scope: 'a', fields: { filter: 'old' } },
        after_state: { kind: 'known', scope: 'a', fields: { filter: 'new' } },
        changed_columns: ['filter'],
      },
    ]),
  ).toEqual([
    {
      resource: 'records',
      operation: 'update',
      before: { kind: 'known', scope: 'a', fields: { filter: 'old' } },
      after: { kind: 'known', scope: 'a', fields: { filter: 'new' } },
      changedColumns: ['filter'],
    },
  ]);
});
...
describe('language neutral conformance', () => {
  ...
  const schema = (name: string) =>
    ajv.compile(
      JSON.parse(
        readFileSync(
          new URL('../../spec/server-driven-impact/schemas/' + name + '.schema.json', import.meta.url),
          'utf8',
        ),
      ),
    );
  const validFact = schema('write-fact'),
    validManifest = schema('query-manifest'),
    validImpact = schema('impact-set');
  for (const file of readdirSync(new URL('../../spec/server-driven-impact/fixtures/', import.meta.url))) {
    const fixture = JSON.parse(
      readFileSync(new URL('../../spec/server-driven-impact/fixtures/' + file, import.meta.url), 'utf8'),
    );
    it(file, () => {
      expect(validManifest(fixture.manifest)).toBe(true);
      for (const fact of fixture.writes) expect(validFact(fact)).toBe(true);
      expect(validImpact(fixture.impact)).toBe(true);
      const result = createImpact({ resources: fixture.resources, manifest: fixture.manifest }).explain(
        fixture.writes,
        fixture.scope,
      );
      expect(validImpact(result.impact)).toBe(true);
      expect(result.impact).toEqual(fixture.impact);
      for (const reason of fixture.reasons) expect(result.decisions.some(d => d.reason === reason)).toBe(true);
    });
  }
});
describe('bounded facts and conservative impact', () => {
  ...
  it('copies input and output, closes contexts, isolates requests, caps facts and bytes', () => {
    ...
    const original = fact('x');
    writes.add([original]);
    original.after = known('mutated');
    expect(writes.snapshot()[0].after).toEqual(known('x'));
    const snapshot = writes.snapshot();
    snapshot[0].after = known('changed');
    expect(writes.snapshot()[0].after).toEqual(known('x'));
    ...
    expect(new WriteSet().snapshot()).toEqual([]);
    writes.close();
    expect(() => writes.add([])).toThrow('WRITE_CONTEXT_CLOSED');
  });
  it('selector and response byte budgets widen without dropping endpoints', () => {
    expect(
      engine
        .explain(
          Array.from({ length: 101 }, (_, i) => fact(i)),
          'a',
        )
  ...
  it('canonicalization is independent of object insertion order and preserves missing/null', () => {
    expect(canonical({ b: 1, a: null })).toBe(canonical({ a: null, b: 1 }));
    expect(canonical({})).not.toBe(canonical({ a: null }));
  });
    ...
    const queries = {
      source: {
        ...
        plan: q.select('records', {
          columns: ['value'],
          where: [q.eq('filter', q.input('filter'))],
          order: [{ field: 'sort' }],
        }),
      },
      mapped: { input: { parse: (v: unknown) => v }, plan: q.call('source', () => ({ filter: 1 })) },
    };
    const graph = compileManifest(queries, resources);
    expect(graph.reads.source[0].columns).toEqual(['filter', 'sort', 'value']);
  ...
  it('preserves bindings through an identity call and widens an arbitrary call mapping', () => {
    const source = {
      input: { parse: (v: unknown) => v },
      plan: q.select('records', { where: [q.eq('filter', q.input('filter'))] }),
    };
    const graph = compileManifest(
      {
        source,
        identity: { input: source.input, plan: q.call('source') },
        mapped: { input: source.input, plan: q.call('source', () => ({ filter: 1 })) },
      },
      resources,
    );
    ...
    const graph = compileManifest(
      {
        ...
        broad: {
          input,
          plan: q.select('records', {
            where: [q.or(q.eq('filter', q.input('filter')), q.eq('value', q.literal('public')))],
          }),
        },
      },
      resources,
