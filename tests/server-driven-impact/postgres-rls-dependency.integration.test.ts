...
describe.skipIf(!enabled)('RLS hidden row and column dependencies', () => {
  ...
  it('rejects a q projection that omits a same-row RLS column while permitting explicit coverage', async () => {
    ...
    const covered = compileManifest(
      { simple: { input, plan: q.select('simple', { columns: ['id', 'visible'] }) } },
      resources,
    );
    ...
    const wholeRow = compileManifest(
      { simple: { input, plan: q.select('simple', { columns: ['visible'] }) } },
      resources,
    );
  ...
  it('requires broad hidden resource paths and disables literal proofs regardless of catalog iteration order', async () => {
    ...
    const broad: QueryManifest = {
      reads: {
        visible: [
          ...
          { resource: 'controls', columns: '*', bindings: [], filters: [{ column: 'status', value: 'visible' }] },
        ],
      },
    };
    ...
    const queries = {
      visible: {
        input,
        plan: {
          kind: 'postgres-query' as const,
          text: `select id from "${schema}".secured`,
          parameters: [],
          reads: broad.reads.visible,
        },
      },
    };
    ...
    const changed = await engine.command({ scope: null }, tx =>
  ...
  it('rejects same-table helper bindings and unproved cross-tenant hidden dependencies', async () => {
    const resources: Resources = {
      selfread: { schema, table: 'selfread', idColumn: 'id', scopeColumn: null, columns: ['id', 'status'] },
    };
    const narrow = compileManifest(
      { selfread: { input, plan: q.select('selfread', { where: [q.eq('id', q.input('id'))] }) } },
      resources,
    );
    ...
    const inline: Resources = {
      inline: { schema, table: 'inline_self', idColumn: 'id', scopeColumn: null, columns: ['id', 'status'] },
    };
    const inlineBound = compileManifest(
      { inline: { input, plan: q.select('inline', { where: [q.eq('id', q.input('id'))] }) } },
      inline,
    );
    ...
    await expect(
      validateCatalog(admin, scoped, {
        reads: {
          visible: [
            { resource: 'secured', columns: '*', bindings: [] },
            { resource: 'controls', columns: '*', bindings: [] },
          ],
        },
      }),
    ).rejects.toThrow('UNRESOLVED_RLS_SCOPE_DEPENDENCY');
  });
});

describe.skipIf(!enabled)('RLS command, role and column precision', () => {
  const schema = 'sdi_policy_' + randomUUID().replaceAll('-', '');
  const roleA = schema + '_a',
    roleB = schema + '_b',
    roleC = schema + '_c',
    owner = schema + '_owner',
    bypass = schema + '_bypass';
  const admin = enabled ? postgres(adminUrl!, { max: 1, prepare: false, onnotice: () => {} }) : undefined!;
  const pool =
    enabled && process.env.SDI_POSTGRES_DRIVER === 'pg'
      ? new Pool({ connectionString: runtimeUrl, max: 1 })
  ...
  const resources: Resources = Object.fromEntries(
    [
      ['profile', ['uid', 'superuser', 'birthday']],
      ['badge', ['id', 'owner_id', 'visible']],
      ['secured', ['id', 'owner_id', 'visible']],
      ['helper', ['id', 'owner_id', 'visible']],
      ['atomic', ['id', 'owner_id', 'visible']],
      ['checked', ['id', 'owner_id', 'visible']],
      ['roles', ['id', 'owner_id', 'visible']],
      ['owned', ['id', 'owner_id', 'visible']],
      ['opaque', ['id', 'owner_id', 'visible']],
    ].map(([table, columns]) => [
      table,
      { schema, table, idColumn: table === 'profile' ? 'uid' : 'id', scopeColumn: null, columns },
    ]),
  ) as Resources;
  const definition = (table: string, suffix = '') => ({
    input,
    source: { text: `select id from "${schema}"."${table}" ${suffix}` },
    onUnresolved: 'reject' as const,
  });
  const compile = (definitions: Parameters<typeof compilePostgresArtifacts>[2], effectiveRole?: string) =>
    compilePostgresArtifacts(admin, resources, definitions, { version, searchPath: [schema, 'public'], effectiveRole });
  beforeAll(async () => {
    version = Math.floor(
  ...
  afterAll(async () => {
    await database.end();
    await admin.unsafe(
      `drop schema if exists "${schema}" cascade; drop role "${roleB}","${roleC}","${roleA}","${owner}","${bypass}"`,
    );
    await admin.end();
  });
  it('excludes UPDATE and WITH CHECK reads, while locking SELECT retains UPDATE USING', async () => {
    const artifact = await compile(
      {
        ...
        locked: definition('badge', 'for share'),
        checkedLocked: definition('checked', 'for update'),
      },
      'routine_runtime',
    );
    expect(artifact.manifest.reads.badge).toEqual([{ resource: 'badge', columns: ['id'], bindings: [] }]);
    expect(artifact.manifest.reads.checked).toEqual([
      { resource: 'checked', columns: ['id', 'visible'], bindings: [] },
    ]);
    expect(artifact.manifest.reads.checkedLocked).toEqual([
      { resource: 'checked', columns: ['id', 'visible'], bindings: [] },
    ]);
    expect(artifact.manifest.reads.locked).toContainEqual({
      resource: 'profile',
      columns: ['superuser', 'uid'],
      bindings: [],
    });
    ...
    await expect(
      admin.unsafe(`create policy invalid on "${schema}".badge for select with check(true)`),
    ).rejects.toMatchObject({ code: '42601' });
    const minimal = await compilePostgresArtifacts(
      admin,
      { badge: resources.badge },
      { badge: definition('badge') },
      { version, searchPath: [schema], effectiveRole: 'routine_runtime' },
    );
  ...
  it('compares actual SELECT results and impacts for unrelated UPDATE, gate INSERT/UPDATE/DELETE and SQL helpers', async () => {
    ...
    for (const endpoint of ['secured', 'helper', 'atomic'])
      expect(artifact.manifest.reads[endpoint]).toContainEqual({
        resource: 'profile',
        columns: ['superuser', 'uid'],
        bindings: [],
      });
    expect(artifact.manifest.reads.secured).toContainEqual({
      resource: 'secured',
      columns: ['id', 'owner_id'],
      bindings: [],
    });
    ...
    expect(await check(sql`update ${identifier(schema)}.profile set superuser=false`)).toEqual([
      'atomic',
      'helper',
      'secured',
    ]);
    expect(await check(sql`update ${identifier(schema)}.profile set superuser=true`)).toEqual([
      'atomic',
      'helper',
      'secured',
    ]);
  ...
  it('selects effective roles and inherited membership, retaining permissive and restrictive dependencies', async () => {
    ...
    expect(inherited.manifest.reads.roles).toContainEqual({
      resource: 'profile',
      columns: ['superuser', 'uid'],
      bindings: [],
    });
    ...
    const engine = createImpact({
      adapter: postgresAdapter({
        database,
        setup: async tx => {
          await tx.unsafe(`set local role "${roleB}"`);
        },
      }),
      resources,
      queries: inherited.queries,
    });
    await engine.validate();
  ...
  it('honors owner, FORCE RLS and BYPASSRLS, and rejects stale definitions', async () => {
    const readAs = (role: string) =>
      admin.begin(async tx => {
        await tx.unsafe(`set local role "${role}"`);
        return [...(await tx.unsafe(`select id from "${schema}".owned`))];
      });
    const ownerArtifact = await compile({ owned: definition('owned') }, owner);
    expect(
      ownerArtifact.queries.owned.plan.kind === 'postgres-query' &&
        ownerArtifact.queries.owned.plan.policyProof?.dependencies,
    ).toEqual([]);
    ...
    expect(
      forced.queries.owned.plan.kind === 'postgres-query' && forced.queries.owned.plan.policyProof?.dependencies,
    ).toHaveLength(1);
    ...
    for (const role of [bypass, 'postgres']) {
      ...
      expect(
        artifact.queries.owned.plan.kind === 'postgres-query' && artifact.queries.owned.plan.policyProof?.dependencies,
      ).toEqual([]);
  ...
  it('rejects unknown function resources or marks the query no-store', async () => {
    ...
    expect(artifact.queries.opaque.plan.kind === 'postgres-query' && artifact.queries.opaque.plan.cache).toBe(
      'no-store',
    );
  });
  it('uses the SECURITY DEFINER owner inside helpers and retains external INSERT/DELETE reads', async () => {
    ...
    try {
      const artifact = await compile(
        { secured: definition('secured'), helper: definition('helper') },
        'routine_runtime',
      );
      await admin.unsafe(generateObserverMigration(resources, artifact.manifest, { runtimeRole: 'routine_runtime' }));
      const engine = createImpact({ adapter: postgresAdapter({ database }), resources, queries: artifact.queries });
      ...
      const proof =
        artifact.queries.helper.plan.kind === 'postgres-query' ? artifact.queries.helper.plan.policyProof : undefined;
      expect(proof?.dependencies.filter(read => read.resource === 'profile')).toEqual([
        { resource: 'profile', columns: ['superuser', 'uid'], rowConstraint: 'all' },
      ]);
    } finally {
      await admin.unsafe(
        `delete from "${schema}".profile; drop policy hidden_profile on "${schema}".profile; alter table "${schema}".profile disable row level security`,
      );
    }
  });
  it('does not mistake the runtime search_path for the deparser search_path', async () => {
    await admin.begin(async tx => {
      ...
      const artifact = await compilePostgresArtifacts(
        ...
        resources,
        { secured: definition('secured') },
        { version, searchPath: ['public'], effectiveRole: 'routine_runtime' },
      );
      expect(artifact.manifest.reads.secured).toContainEqual({
        resource: 'profile',
        columns: ['superuser', 'uid'],
  ...
  it('does not reuse a definer proof for an independent structured read in a composed endpoint', async () => {
    ...
    try {
      ...
      const covered = compileManifest(
        { mixed: { input, plan: q.combine({ ...children, gate: q.select('badge', { columns: ['visible'] }) }) } },
        resources,
      );
      ...
      const mixedNative = await compilePostgresArtifacts(
        admin,
        scoped,
        {
          mixed: {
            input,
            source: { text: `select uid,"${schema}".profile_count() from "${schema}".profile` },
            onUnresolved: 'reject',
          },
        },
        { version, searchPath: [schema, 'public'], effectiveRole: bypass },
      );
      await expect(validateCatalog(admin, scoped, mixedNative.manifest)).rejects.toThrow(
        'UNRESOLVED_RLS_SCOPE_DEPENDENCY',
      );
    } finally {
      await admin.unsafe(
        `drop policy hidden_profile on "${schema}".profile; alter table "${schema}".profile disable row level security`,
      );
    }
  });
  ...
  it('does not replace session/time freshness with broad row dependencies', async () => {
    await admin.unsafe(
      `create policy clock_policy on "${schema}".checked for select using(current_timestamp is not null)`,
    );
    await expect(compile({ checked: definition('checked') })).rejects.toThrow(
      'POSTGRES_QUERY_REQUIRES_FRESHNESS_POLICY',
    );
    const artifact = await compile({ checked: { ...definition('checked'), onUnresolved: 'no-store' } });
    expect(artifact.queries.checked.plan.kind === 'postgres-query' && artifact.queries.checked.plan.cache).toBe(
      'no-store',
    );
  });
});
