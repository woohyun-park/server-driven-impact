...
describe('Query Plan precision with real SQLite result comparisons', () => {
  it('ignores count-only projection, order and optional joins while retaining row membership', async () => {
    ...
    try {
      database.exec(
        'create table orders(id text primary key,customer text,note text,sort integer); create table items(id text primary key,order_id text,amount integer);',
      );
      const options = {
        columns: ['note'],
        where: [q.eq('customer', q.input('customer'))],
        order: [{ field: 'sort' }],
        joins: [{ as: 'items', resource: 'items', local: 'id', foreign: 'order_id', many: true }],
      };
      const queries = {
        count: { input, plan: q.count('orders', options) },
        required: {
          input,
          plan: q.count('orders', { ...options, joins: options.joins.map(join => ({ ...join, required: true })) }),
        },
      };
      const manifest = compileManifest(queries, resources);
      expect(manifest.sources.count).toEqual(['orders']);
      expect(manifest.reads.count).toEqual([
        { resource: 'orders', columns: ['customer'], bindings: [{ column: 'customer', input: 'customer' }] },
      ]);
  ...
  it('propagates parent equality into nested joins without narrowing unrelated inputs', () => {
    const manifest = compileManifest(
      {
        detail: {
          ...
          plan: q.select('orders', {
            columns: ['id'],
            where: [q.eq('id', q.input('order'))],
            joins: [{ as: 'items', resource: 'items', local: 'id', foreign: 'order_id', columns: ['amount'] }],
          }),
        },
      },
      resources,
    ...
    const changed: WriteFact = {
      resource: 'items',
      operation: 'update',
      before: { kind: 'known', scope: null, fields: { order_id: 'a' } },
      after: { kind: 'known', scope: null, fields: { order_id: 'b' } },
