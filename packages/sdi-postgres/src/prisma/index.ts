...
export interface PrismaCommandClient { $disconnect(): Promise<void> }
...
class CommandPool extends pg.Pool {
  constructor(database: PgCommandDb) {
    ...
    this.connect = (() => { throw new Error('PRISMA_COMMAND_CONNECTION_OWNED_BY_SDI'); }) as pg.Pool['connect'];
  }
}
...
function bindClient(database: PgCommandDb, schema?: string) {
  ...
  function assertOpen() { if (!open) throw new Error('WRITE_CONTEXT_CLOSED'); }
  ...
  function factory(db: PgCommandDb): SqlDriverAdapterFactory {
    return {
      provider: 'postgres', adapterName: 'sdi-prisma-pg',
      async connect(): Promise<SqlDriverAdapter> {
        assertOpen();
        const pool = new CommandPool(db);
        // Prisma and this package may resolve different @types/pg minor versions.
        const inner = await new PrismaPg(pool as unknown as ConstructorParameters<typeof PrismaPg>[0], { schema }).connect();
        ...
        return {
          ...
          executeScript: async () => { throw new Error('PRISMA_COMMAND_SCRIPT_UNSUPPORTED'); },
          ...
          async startTransaction(isolationLevel): Promise<Transaction> {
            ...
            const ready = new Promise<SqlDriverAdapter>((resolve, reject) => { readyResolve = resolve; readyReject = reject; });
            let commit!: () => void;
            let abort!: (reason: unknown) => void;
            const gate = new Promise<void>((resolve, reject) => { commit = resolve; abort = reject; });
            void gate.catch(() => {});
            ...
            void completion.then(() => transactions.delete(transaction), error => { transactions.delete(transaction); readyReject(error); });
            ...
            return {
              ...
              async commit() { commit(); await completion; },
              async rollback() {
                abort(rollback);
                try { await completion; } catch (error) { if (error !== rollback) throw error; }
              },
            };
          },
        };
  ...
  return {
    ...
    finish() { if (operations.size || transactions.size) throw new Error('UNAWAITED_DATABASE_OPERATION'); },
    async close() {
      open = false;
...
export function prismaAdapter<Client extends PrismaCommandClient>(options: PrismaOptions<Client>): ImpactAdapter<Client> {
  ...
  return {
    [bindAdapter](resources, manifest) {
      ...
      return {
        ...bound,
        command: (scope, writes, work) => bound.command(scope, writes, async database => {
