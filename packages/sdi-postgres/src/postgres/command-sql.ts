...
export async function assertCommandSql(text:string) {
  const parser=require('@pgsql/parser/v18') as {parse(text:string):Promise<{stmts?:{stmt:Record<string,unknown>}[]}>};
  ...
  const allowed=new Set(['SelectStmt','InsertStmt','UpdateStmt','DeleteStmt','MergeStmt','TruncateStmt','CopyStmt','CallStmt','DoStmt','ConstraintsSetStmt']);
