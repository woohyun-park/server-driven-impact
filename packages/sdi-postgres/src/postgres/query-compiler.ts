...
function node(value: unknown, kind: string): Json | undefined {
  if (
    ...
    typeof value === 'object' &&
    'op' in value &&
    String((value as Json).op).startsWith('SETOP_')
  )
...
function stringNode(value: unknown): string | undefined {
  const string = node(value, 'String');
  return string?.sval ?? string?.str;
}
function aliasOf(range: Json): string {
  return node(range.alias, 'Alias')?.aliasname ?? range.alias?.aliasname ?? range.relname;
}
 ...
 */
export async function compilePostgresQuery(
  source: PostgresQuerySource,
  resources: Resources,
  version: PostgresMajor,
  options: { catalog?: PostgresCatalogResolver } = {},
): Promise<PostgresQueryPlan> {
  validateResources(resources);
  if (!source || typeof source.text !== 'string' || !source.text.trim() || source.text.length > 1_048_576)
    throw new Error('INVALID_POSTGRES_QUERY');
  ...
  try {
    parser = require(`@pgsql/parser/v${version}`) as typeof parser;
  } catch (cause) {
    throw new Error(`POSTGRES_PARSER_UNAVAILABLE:${version}`, { cause });
  }
  ...
  if (usedParameters.size !== parameters.length || parameters.some((_, index) => !usedParameters.has(index + 1)))
    throw new Error('POSTGRES_QUERY_PARAMETER_MISMATCH');
  if (source.cache !== undefined && source.cache !== 'no-store') throw new Error('INVALID_POSTGRES_CACHE_POLICY');
  if (source.cache === 'no-store')
  ...
  function collectReferences(value: unknown, ctes: Set<string>): void {
    ...
    if (select) {
      collectSelectReferences(select, ctes);
      return;
    }
    const range = node(value, 'RangeVar');
    if (range && (range.schemaname || !ctes.has(range.relname))) {
      const reference = relationReference(range);
      relationReferences.set(canonical(reference), reference);
    }
    ...
    if (call && Array.isArray(call.funcname)) {
      ...
      if (names.every(Boolean) && names.length <= 2) {
        const reference = {
          ...
          name: names.at(-1)!,
          arguments: Array.isArray(call.args) ? call.args.length : 0,
        };
  ...
  const catalogRelations = new Map<
    string,
    { resources: readonly string[]; direct?: string; policies?: readonly CatalogPolicyDependency[] }
  >();
  ...
  for (const reference of relationReferences.values()) {
    const direct = reference.schema
      ? Object.entries(resources)
          .filter(
            ([, resource]) => (resource.schema ?? 'public') === reference.schema && resource.table === reference.name,
          )
          .map(([id]) => id)
      : Object.entries(resources)
          .filter(([, resource]) => resource.table === reference.name)
          .map(([id]) => id);
    ...
    for (const resource of resolved)
      if (direct.length !== 1 || (resource !== direct[0] && !policies?.some(read => read.resource === resource)))
        unprovenPolicyResources.add(resource);
    catalogRelations.set(canonical(reference), {
      resources: resolved,
      policies,
      ...(direct.length === 1 ? { direct: direct[0] } : {}),
    });
  }
  ...
  function resourceFor(range: Json): string {
    ...
    const ids = Object.entries(resources)
      .filter(([, resource]) => resource.table === range.relname)
      .map(([id]) => id);
  ...
  function direct(value: unknown, ctes: Set<string>, relations: DirectRelation[]): void {
    ...
    if (range) {
      ...
      if (resolved) {
        for (const resource of resolved.resources)
          if (resource !== resolved.direct && !resolved.policies?.some(read => read.resource === resource))
            reads.push({ resource, columns: '*', bindings: [] });
        for (const read of resolved.policies ?? [])
          if (read.rowConstraint === 'all')
            reads.push({
              resource: read.resource,
              columns: read.columns === '*' ? '*' : [...read.columns],
              bindings: [],
            });
        if (!resolved.direct) return;
      }
      const sameRow = resolved?.policies?.filter(read => read.rowConstraint === 'same-row');
      const policyColumns = sameRow?.some(read => read.columns === '*')
        ? '*'
        : sameRow?.flatMap(read => [...read.columns]);
      relations.push({
        resource: resourceFor(range),
        alias: aliasOf(range),
        pruneColumns: options.catalog?.canPruneColumns?.(relationReference(range)) === true,
        policyColumns,
      });
      return;
    }
    ...
    if (join) {
      direct(join.larg, ctes, relations);
      direct(join.rarg, ctes, relations);
    }
  }
  ...
  function atomic(value: unknown): Binding | undefined {
    ...
    const leftParameter = unwrapParameter(expression.lexpr),
      rightParameter = unwrapParameter(expression.rexpr);
  ...
  function guaranteed(value: unknown): Binding[] {
    ...
    if (bool) {
      ...
      if (bool.boolop === 'OR_EXPR' && branches.length)
        return branches[0].filter(binding =>
          branches.slice(1).every(branch => branch.some(candidate => canonical(candidate) === canonical(binding))),
        );
      return [];
    }
    const binding = atomic(value);
  ...
  function observedColumns(select: Json, relations: DirectRelation[]): Map<DirectRelation, Set<string>> | undefined {
    ...
    function inspect(value: unknown): void {
      ...
      if (node(value, 'SelectStmt')) {
        complete = false;
        return;
      }
      const join = node(value, 'JoinExpr');
      if (join && (join.isNatural || join.usingClause?.length)) {
        complete = false;
        return;
      }
      if (node(value, 'ColumnRef')) {
        ...
        const owners = relations.filter(
          relation =>
            (!reference.alias || reference.alias === relation.alias) &&
            resources[relation.resource].columns.includes(reference.column),
        );
        if (owners.length !== 1) {
          complete = false;
          return;
  ...
  function nested(value: unknown, ctes: Set<string>): void {
    if (Array.isArray(value)) {
      for (const item of value) nested(item, ctes);
      return;
    }
    if (!value || typeof value !== 'object') return;
    if ((!options.catalog && 'FuncCall' in value) || 'RangeFunction' in value || 'TableFunc' in value)
      unresolvedFunction = true;
    if ('SQLValueFunction' in value || 'NextValueExpr' in value) nonDataDependency = true;
    const select = node(value, 'SelectStmt');
    if (select) {
      visitSelect(select, ctes);
      return;
    }
  ...
  function visitSelect(select: Json, inherited: Set<string>): void {
    ...
    for (const relation of relations) {
      ...
      const resolved = bindings.flatMap(binding => {
        ...
        if (binding.alias) {
          if (
            binding.alias !== relation.alias ||
            aliases.get(binding.alias)?.length !== 1 ||
            !resource.columns.includes(binding.column)
          )
            return [];
        } else {
      ...
      reads.push({
        ...
        columns:
          relation.pruneColumns && observed && relation.policyColumns !== '*'
            ? [...new Set([...observed.get(relation)!, ...(relation.policyColumns ?? [])])].sort()
            : '*',
        bindings: [...new Map(resolved.map(binding => [canonical(binding), binding])).values()],
      });
    }
    for (const [key, value] of Object.entries(select))
      if (!['fromClause', 'whereClause', 'withClause'].includes(key)) nested(value, ctes);
  ...
  (function collect(value: unknown): void {
    ...
    if (!value || typeof value !== 'object') return;
    const parameter = node(value, 'ParamRef')?.number;
    if (typeof parameter === 'number') allParameters.add(parameter);
    Object.values(value).forEach(collect);
  })(root);
  if (allParameters.size !== parameters.length || parameters.some((_, index) => !allParameters.has(index + 1)))
    throw new Error('POSTGRES_QUERY_PARAMETER_MISMATCH');
  ...
  for (const read of reads) {
    ...
    if (!previous) {
      merged.set(key, { ...read, bindings });
      continue;
    }
    previous.columns =
      previous.columns === '*' || read.columns === '*'
        ? '*'
        : [...new Set([...previous.columns, ...read.columns])].sort();
  }
  const policyProof = options.catalog?.policyDependencies
    ? {
        resources: [
          ...new Set(
            [...catalogRelations.values()].flatMap(relation => [
              ...(relation.direct ? [relation.direct] : []),
              ...(relation.policies ?? []).map(read => read.resource),
            ]),
          ),
        ]
          .filter(resource => !unprovenPolicyResources.has(resource))
          .sort(),
  ...
  return Object.freeze({
    ...
    parameters: Object.freeze(parameters),
    reads: Object.freeze(
      [...merged.values()].map(read => Object.freeze({ ...read, bindings: Object.freeze(read.bindings) })),
    ),
    ...(policyProof ? { policyProof } : {}),
  }) as PostgresQueryPlan;
}
