...
export function observerFingerprint(resources:Resources,manifest:QueryManifest):string {
  return createHash('sha256').update(canonical({observerProtocol:9,resources,reads:manifest.reads,...(manifest.postgres?{postgres:manifest.postgres}:{})})).digest('hex');
}
function observedColumns(resourceId:string,resource:Resources[string],manifest:QueryManifest):string[] {
  ...
  return [...new Set([
    ...
    ...reads.flatMap(read=>[...read.bindings.map(binding=>binding.column), ...(read.filters ?? []).map(filter=>filter.column)]),
  ])].sort();
}
function rowState(alias:string,resource:Resources[string],columns:readonly string[],filterColumns:readonly string[]):string {
  ...
  const equality = filterColumns.map(column => `case when pg_typeof(${alias}.${identifier(column)})::oid in (16,20,21,23,25,1043) then jsonb_build_object(${literal(column)},to_jsonb(${alias}.${identifier(column)})) else '{}'::jsonb end`).join(' || ');
...
function batchState(relation: string, alias: string, resource: Resources[string], columns: readonly string[], filterColumns: readonly string[]): string {
  ...
  const value = (column: string) => `coalesce(to_jsonb(${alias}.${identifier(column)}),'null'::jsonb)::text collate "C"`;
  const scope = resource.scopeColumn === null ? `'null'::text` : value(resource.scopeColumn);
  const fields = columns.map(column => `case when min(${value(column)})=max(${value(column)}) then jsonb_build_object(${literal(column)},min(${value(column)})::jsonb) else '{}'::jsonb end`).join(' || ') || "'{}'::jsonb";
  const equality = filterColumns.map(column => `case when min(pg_typeof(${alias}.${identifier(column)})::oid::int) in (16,20,21,23,25,1043) and min(${value(column)})=max(${value(column)}) then jsonb_build_object(${literal(column)},min(${value(column)})::jsonb) else '{}'::jsonb end`).join(' || ');
...
function triggerFunction(internalSchema:string,resourceId:string,operation:'insert'|'update'|'delete'|'truncate',resource:Resources[string],columns:readonly string[],filterColumns:readonly string[]):string {
  ...
  const insertBatch = (before: string, after: string) => `insert into pg_temp.${collectorTable}(token,resource,operation,before_state,after_state,changed_columns)
...
export function generateObserverMigration(resources:Resources,manifest:QueryManifest,options:{runtimeRole?:string}={}):string {
  ...
  for(const [resourceId,resource] of Object.entries(resources)) {
    ...
    for(const operation of ['insert','update','delete','truncate'] as const) {
      ...
      statements.push(triggerFunction(internalSchema,resourceId,operation,resource,columns,[...new Set(Object.values(manifest.reads).flat().filter(read=>read.resource===resourceId).flatMap(read=>(read.filters ?? []).map(filter=>filter.column)))]));
...
function state(value:unknown):RowState {
  ...
  const equalityFields = row.equalityFields && typeof row.equalityFields === 'object' && !Array.isArray(row.equalityFields) && Object.values(row.equalityFields).every(isScalar) ? row.equalityFields as Record<string,Scalar> : undefined;
  return {kind:'known',scope:row.scope,fields:row.fields as Record<string,Scalar>,...(equalityFields ? {equalityFields} : {})};
}
