...
export function sqlReferences(tree: unknown) {
  ...
  function visit(value:unknown, inherited:Set<string>) {
    if(Array.isArray(value)){value.forEach(child=>visit(child,inherited));return;}
    ...
    if(select) {
      ...
      for(const cte of ctes){visit(cte.ctequery,new Set(visible));visible.add(cte.ctename);}
    ...
    if(call?.funcname) {
      ...
      if(names.every(Boolean) && names.length<=2){
        const reference={...(names.length===2?{schema:names[0]}:{}),name:names.at(-1),arguments:call.args?.length??0};
