...
export class TrackedDb<TResult = PostgresExecuteResult> {
  ...
  constructor(
    ...
    scope: Scalar,
    resources: Resources,
    private readonly executeStatement: (statement: Sql, options?: DriverQueryOptions) => Promise<TResult> = statement => tx.unsafe(statement.text,statement.values as never[]) as unknown as Promise<TResult>,
  ...
  async query(query: string | {text: string; values?: readonly unknown[]; rowMode?: 'array'; types?: unknown; name?: string}, values?: readonly unknown[]): Promise<TResult> {
    if (!this.open) throw new Error('WRITE_CONTEXT_CLOSED');
    if (query && typeof query === 'object' && Object.keys(query).some(key => !['text','values','rowMode','types','name'].includes(key))) throw new Error('UNSUPPORTED_QUERY_OPTION');
    const config = typeof query === 'string' ? {text: query, values} : {...query, values: values ?? query?.values};
    if (!config || typeof config.text !== 'string' || (config.values !== undefined && !Array.isArray(config.values))) throw new Error('INVALID_QUERY_CONFIG');
    // Snapshot before async validation so callers cannot switch SQL after it passed.
    const text = config.text, parameters = [...(config.values ?? [])];
  ...
  async *cursor(statement: Sql, batchSize=100): AsyncIterable<Row[]> {
    ...
    if (!this.open) throw new Error('WRITE_CONTEXT_CLOSED');
    const text = statement.text, values = [...statement.values];
    await assertCommandSql(text);
