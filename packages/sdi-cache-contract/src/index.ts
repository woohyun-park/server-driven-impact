...
import type { CacheCompileOptions, CacheContract, CacheContractReference, CacheDiagnosticReason, CacheEndpoint, CacheInvalidation, CacheInvalidationSet, CacheQueryContract, CacheValue, QueryKey, StringComparisonStatus } from './types.js';
...
export function defineCacheContract<const T extends CacheContract>(contract: T): T {
  ...
  if (!Number.isSafeInteger(contract.version) || contract.version < 1) throw new Error('INVALID_CACHE_CONTRACT_VERSION');
  ...
  for (const query of snapshot.queries) {
    ...
    if (query.invalidation !== undefined && !['preserve', 'endpoint'].includes(query.invalidation)) throw new Error('INVALID_CACHE_INVALIDATION_POLICY');
    ...
    if (query.pageInput) {
      ...
      if (Object.keys(query.pageInput).some(field => own(query.input,field))) throw new Error('CACHE_PAGE_INPUT_OVERLAP');
    }
  ...
  if (snapshot.excludedEndpoints !== undefined && !Array.isArray(snapshot.excludedEndpoints)) throw new Error('INVALID_CACHE_EXCLUSIONS');
  for (const entry of snapshot.excludedEndpoints ?? []) {
    ...
    if (excluded.has(entry.endpoint) || snapshot.queries.some(query => query.endpoint === entry.endpoint)) throw new Error('DUPLICATE_CACHE_ENDPOINT_POLICY:' + entry.endpoint);
...
export function validateCacheContractCoverage(candidate: CacheContract, endpoints: readonly CacheEndpoint[]): void {
  ...
  for (const endpoint of endpoints) {
    ...
    if (endpoint.cache === 'no-store' && registered) throw new Error('CACHE_ENDPOINT_REQUIRES_NO_STORE:' + endpoint.endpoint);
    if (endpoint.cache === 'cacheable' && excluded?.reason === 'no-store') throw new Error('CACHE_ENDPOINT_NOT_NO_STORE:' + endpoint.endpoint);
  }
...
function normalize(query: CacheQueryContract, input: Record<string, unknown>): Record<string, CacheValue> {
  ...
  if (!('template' in query.key)) {
    ...
    if (params.length && query.key.paramsAnchor?.some(field => !own(normalized,field))) throw new Error('CACHE_PARAMS_ANCHOR_REQUIRED');
  }
  return normalized;
}
export function normalizeCacheInput(contract: CacheContract, operationId: string, input: Record<string, unknown>): Record<string, CacheValue> {
  return normalize(operation(contract,operationId),input);
}
/** Send this normalized input to the API as well as using its queryKey. */
export function prepareCacheQuery(contract: CacheContract, operationId: string, input: Record<string, unknown>): {input: Record<string, CacheValue>; queryKey: QueryKey} {
...
export function buildQueryExecutionInput(contract: CacheContract, operationId: string, input: Record<string, unknown>, pageInput: Record<string, unknown>): Record<string, CacheValue> {
  const query = operation(contract,operationId);
  if (query.kind !== 'infinite') throw new Error('CACHE_PAGE_INPUT_REQUIRES_INFINITE');
...
function forValue(query: CacheQueryContract, value: Record<string, Scalar>, stringComparison?: StringComparison): Decision {
  ...
  for (const [field,expected] of Object.entries(value)) {
    ...
    if (own(definition,'default') || (!definition.required && !anchors.includes(field))) return broad(query,'missing-field');
...
export function validateCacheCompileOptions(options: CacheCompileOptions): void {
  for (const [value,maximum] of [[options.maxInvalidations,CACHE_LIMITS.invalidations],[options.maxBytes,CACHE_LIMITS.responseBytes]]) {
    if (value !== undefined && (!Number.isSafeInteger(value) || value < 1 || value > maximum!)) throw new Error('INVALID_CACHE_COMPILE_LIMIT');
  }
  if (options.explain !== undefined && typeof options.explain !== 'function') throw new Error('INVALID_CACHE_EXPLAIN');
}
function compile(candidate: CacheContract, impact: ImpactSet, scope: Scalar, options: CacheCompileOptions, stringComparison?: StringComparison): CacheInvalidationSet {
  const contract = defineCacheContract(candidate);
  ...
  if (!Array.isArray(impact.targets) || impact.targets.length > LIMITS.endpoints) throw new Error('INVALID_IMPACT_TARGETS');
  const invalidations: CacheInvalidation[] = [];
  const affected = new Set<CacheQueryContract>();
  for (const target of impact.targets) {
    if (!target || typeof target.endpoint !== 'string' || !['caller','global'].includes(target.scope)) throw new Error('INVALID_IMPACT_TARGET');
    const queries = contract.queries.filter(query => query.endpoint === target.endpoint);
    if (!queries.length) {
      if (!contract.excludedEndpoints?.some(entry => entry.endpoint === target.endpoint)) throw new Error('CACHE_ENDPOINT_UNCOVERED:' + target.endpoint);
  ...
  const unique = (values: CacheInvalidation[]) => [...new Map(values.map(value => [value.exact + ':' + canonical(value.queryKey),value])).values()];
  const payload = (values: CacheInvalidation[]): CacheInvalidationSet => ({protocolVersion:1,contractId:contract.id,contractVersion:contract.version,scope,invalidations:unique(values)});
  const fits = (value: CacheInvalidationSet) => value.invalidations.length <= (options.maxInvalidations ?? CACHE_LIMITS.invalidations) && byteLength(value) <= (options.maxBytes ?? CACHE_LIMITS.responseBytes);
...
export function compileCacheInvalidations(candidate: CacheContract, impact: ImpactSet, scope: Scalar, options: CacheCompileOptions = {}): CacheInvalidationSet {
...
export function createCacheContractRegistry(contracts: readonly CacheContract[], options: {
  ...
  return Object.freeze({resolve,compile(reference: CacheContractReference,impact: ImpactSet,scope: Scalar) {
