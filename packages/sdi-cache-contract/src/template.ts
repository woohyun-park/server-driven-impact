...
export function validateTemplate(query: CacheQueryContract): void {
  ...
  function walk(node: CacheKeyNode, depth: number, inArray: boolean, last: boolean): void {
    ...
    switch (node.kind) {
      ...
      case 'inputs':
        ...
        if (node.omitEmpty !== undefined && typeof node.omitEmpty !== 'boolean') throw new Error('INVALID_CACHE_KEY_TEMPLATE');
  ...
  if (!('template' in query.key)) {
    ...
    if (new Set([...path, ...params]).size !== path.length + params.length) throw new Error('DUPLICATE_CACHE_KEY_FIELD');
    if (paramsAnchor && (!paramsAnchor.length || paramsAnchor.some(name => !params.includes(name)))) throw new Error('INVALID_CACHE_PARAMS_ANCHOR');
  }
}
...
function literalShape(value: CacheValue): Shape {
  ...
  if (value && typeof value === 'object') return {kind: 'object', fields: Object.fromEntries(Object.entries(value).map(([k,v]) => [k, literalShape(v)]))};
...
function intersects(a: Shape, b: Shape): boolean {
  ...
  if (a.kind === 'object' && b.kind === 'object') {
    for (const name of new Set([...Object.keys(a.fields),...Object.keys(b.fields)])) {
      ...
      if (!x || !y) { if (!(x ?? y).optional) return false; }
...
export function validateKeySeparation(queries: readonly CacheQueryContract[]): void {
  ...
  for (let i=0;i<queries.length;i++) for (let j=i+1;j<queries.length;j++) {
    if (intersects(shapes[i],shapes[j])) throw new Error('CACHE_KEY_COLLISION:' + queries[i].operationId + ':' + queries[j].operationId);
  }
}
function guarantees(node: Shape, filter: CacheValue): boolean {
  ...
  if (node.kind === 'array') return Array.isArray(filter) &&
...
export function validateFallback(query: CacheQueryContract): void {
  if (!Array.isArray(query.fallback) || !query.fallback.length || !isCacheValue(query.fallback) ||
      !guarantees(shape(queryTemplate(query),query),query.fallback)) throw new Error('CACHE_FALLBACK_NOT_PROVEN:' + query.operationId);
}

const UNKNOWN = Symbol('unknown');
type Render = {value: CacheValue | typeof OMIT | typeof UNKNOWN; complete: boolean};
function render(node: CacheKeyNode, query: CacheQueryContract, input: Record<string, CacheValue>, partial: boolean): Render {
  switch (node.kind) {
...
export function renderKey(query: CacheQueryContract, input: Record<string, CacheValue>, partial = false): {queryKey:QueryKey; exact:boolean} {
