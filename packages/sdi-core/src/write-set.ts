...
export class WriteSet {
  ...
  add(facts: readonly WriteFact[]): void {
    ...
    for (const fact of facts) {
      ...
      if (
        !fact.resource ||
        fact.resource.length > 128 ||
        !['insert', 'update', 'delete', 'unknown'].includes(fact.operation) ||
        !(
          fact.changedColumns === null ||
          (Array.isArray(fact.changedColumns) && fact.changedColumns.every(c => typeof c === 'string'))
        )
      )
        throw new Error('INVALID_WRITE_FACT');
      for (const row of [fact.before, fact.after]) {
        if (!row || !['absent', 'unknown', 'known'].includes(row.kind)) throw new Error('INVALID_ROW_STATE');
        if (
          row.kind === 'known' &&
          row.equalityFields !== undefined &&
          (!row.equalityFields ||
            typeof row.equalityFields !== 'object' ||
            Array.isArray(row.equalityFields) ||
            !Object.values(row.equalityFields).every(isScalar))
        )
          throw new Error('NON_SCALAR_FIELD');
        if (row.kind === 'known' && (!isScalar(row.scope) || !row.fields || !Object.values(row.fields).every(isScalar)))
          throw new Error('NON_SCALAR_FIELD');
      }
      ...
      if (this.overBudget() && !entry.summarized && (entry.facts.length > 1 || entry.bytes > SUMMARY_BYTES))
        this.compact(entry);
      ...
      while (this.overBudget()) {
        const largest = [...this.entries.values()]
          .filter(value => !value.summarized)
  ...
  snapshot(): WriteFact[] {
    return JSON.parse(canonical([...this.entries.values()].flatMap(entry => entry.facts))) as WriteFact[];
  }
  close(): void {
    this.closed = true;
  }
...
function commonRow(left: RowState, right: RowState): RowState {
  ...
  return {
    kind: 'known',
    scope: left.scope,
    fields: commonFields(left.fields, right.fields),
    ...(left.equalityFields && right.equalityFields
      ? { equalityFields: commonFields(left.equalityFields, right.equalityFields) }
      : {}),
  };
}
function commonFields(left: Record<string, Scalar>, right: Record<string, Scalar>) {
  return Object.fromEntries(
    Object.entries(left).filter(([key, value]) => Object.hasOwn(right, key) && right[key] === value),
  );
}
function summarize(facts: readonly WriteFact[]): WriteFact {
  const summary: WriteFact = {
    resource: facts[0].resource,
    operation: facts[0].operation,
    before: { kind: 'absent' },
    after: { kind: 'absent' },
    changedColumns: [],
  };
  for (const fact of facts) {
    ...
    summary.changedColumns =
      summary.changedColumns === null || fact.changedColumns === null
        ? null
        : [...new Set([...summary.changedColumns, ...fact.changedColumns])];
    ...
    if (byteLength(summary) > SUMMARY_BYTES) {
      for (const row of [summary.before, summary.after])
        if (row.kind === 'known') {
          row.fields = {};
          delete row.equalityFields;
        }
