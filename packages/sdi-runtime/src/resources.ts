...
export function identityColumns(resource: Resource): readonly string[] {
  return resource.idColumn === null
    ? []
    : typeof resource.idColumn === 'string'
      ? [resource.idColumn]
      : resource.idColumn;
}
...
export function validateResources(resources: Resources): void {
  ...
  if (entries.length > LIMITS.resources || byteLength(resources) > LIMITS.manifestBytes)
  ...
  for (const [id, resource] of entries) {
    ...
    for (const name of [
      ...
      resource.table,
      ...resource.columns,
      ...(resource.physicalRelations ?? []).flatMap(relation => [relation.schema, relation.table]),
    ]) {
      if (!name || name.includes('\0')) throw new Error('INVALID_IDENTIFIER');
    }
    if (resource.postgresKind !== undefined && resource.postgresKind !== 'materialized-view')
      throw new Error('INVALID_POSTGRES_RESOURCE_KIND');
    if (resource.postgresKind === 'materialized-view' && resource.physicalRelations)
      throw new Error('INVALID_PHYSICAL_RELATIONS');
    if (
      resource.physicalRelations &&
      (!resource.physicalRelations.length ||
        new Set(resource.physicalRelations.map(relation => `${relation.schema}.${relation.table}`)).size !==
          resource.physicalRelations.length)
    )
      throw new Error('INVALID_PHYSICAL_RELATIONS');
    ...
    for (const name of [
      ...
      ...(resource.selectorColumns ?? []),
    ]) {
      if (!resource.columns.includes(name)) throw new Error('UNREGISTERED_COLUMN');
    }
    for (const child of resource.cascades ?? []) {
      if (!Object.hasOwn(resources, child.resource) || !resources[child.resource].columns.includes(child.column))
        throw new Error('INVALID_CASCADE');
    }
  }
