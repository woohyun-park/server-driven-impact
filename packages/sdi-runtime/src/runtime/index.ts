...
export interface CommandInvalidationResult<T> extends CommandResult<T> { cacheInvalidation: CacheInvalidationSet }
...
function parseInput(definition: QueryDefinition, value: unknown, exactStringInputs: readonly string[] = []): Input {
  ...
  if (exactStringInputs.length) {
    if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('QUERY_INPUT_PRESERVATION_VIOLATION');
    for (const field of exactStringInputs) {
      if (Object.hasOwn(value,field) !== Object.hasOwn(input,field) ||
        (Object.hasOwn(value,field) && (value as Record<string,unknown>)[field] !== (input as Record<string,unknown>)[field])) {
...
export function createImpact<Db, Q extends Record<string, QueryDefinition>>(options: {
  adapter: ImpactAdapter<Db>; resources: Resources; queries: Q; cacheContracts?: readonly CacheContract[];
  ...
  const cacheContracts = createCacheContractRegistry(configuredCacheContracts, {
    endpoints: Object.entries(queries).map(([endpoint,query]) => ({
      endpoint, cache: requiresNoStore(query.plan,queries) ? 'no-store' : 'cacheable',
    })),
    ...
    stringComparison(endpoint,input) {
      return adapter.verifiedStringComparisons?.().some(proof => proof.endpoint === endpoint && proof.input === input)
        ? 'verified' : 'comparison-unproven';
    },
  });
  const calculator = createCalculator({ resources, manifest });
  const enabledStringInputs = new Map<string,Set<string>>();
  for (const contract of configuredCacheContracts) for (const query of contract.queries) {
    const fields=enabledStringInputs.get(query.endpoint) ?? new Set<string>();
    for (const [field,definition] of Object.entries(query.input)) if (
  ...
  const exactStringInputs = (endpoint:string) => {
    ...
    return adapter.verifiedStringComparisons?.().filter(proof => proof.endpoint === endpoint && enabled.has(proof.input)).map(proof => proof.input) ?? [];
  };
  ...
  function command<T>(context: Context, work: (db: Db) => Promise<T>, output: CommandOptions & {cacheContract: CacheContractReference}): Promise<CommandInvalidationResult<T>>;
  function command<T>(context: Context, work: (db: Db) => Promise<T>, output?: {cacheContract?: undefined}): Promise<CommandResult<T>>;
  function command<T>(context: Context, work: (db: Db) => Promise<T>, output: CommandOptions | undefined): Promise<CommandResult<T> | CommandInvalidationResult<T>>;
  async function command<T>(context: Context, work: (db: Db) => Promise<T>, output: CommandOptions = {}): Promise<CommandResult<T> | CommandInvalidationResult<T>> {
