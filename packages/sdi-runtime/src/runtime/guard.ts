      ...
      result[key] = (...args: unknown[]) => {
        ...
        if (key === 'savepoint') {
          ...
          args[0] = async (child: object) => {
            ...
            try {
              const data = await callback(guarded.db);
              guarded.finish();
              return data;
            } finally {
              guarded.close();
              await guarded.settle();
              children.delete(guarded);
            }
          };
        }
        ...
        try {
          ...
          pending--;
          if (key === 'savepoint') savepoint = false;
          if (key !== 'savepoint') failed = true;
          throw error;
        }
        if (synchronous) {
          pending--;
          return options.syncFactories?.has(key) && returned && typeof returned === 'object'
            ? wrap(returned)
            : returned;
        }
        if (returned && typeof returned === 'object' && Symbol.asyncIterator in returned) {
          ...
          return Object.freeze({
            ...
            async next(...values: [] | [unknown]) {
              ...
              try {
                const next = await iterator.next(...values);
                if (next.done) finishIterator();
                return next;
              } catch (error) {
                if (key !== 'savepoint') failed = true;
                finishIterator();
            ...
            async throw(error?: unknown) {
              try {
                return iterator.throw ? await iterator.throw(error) : Promise.reject(error);
              } finally {
                if (key !== 'savepoint') failed = true;
                finishIterator();
              }
