...
0.1 버전은 Node 동기 SQLite 드라이버, `main` 데이터베이스, 단일 primary key를 가진 일반 테이블, 등록된 `TEXT`, `INTEGER`, `REAL` 컬럼과 SQLite 기본 `BINARY`·`NOCASE`·`RTRIM` collation을 지원합니다. `ATTACH`, virtual table, generated/hidden column, 사용자 정의 collation, `REPLACE`, `INSERT OR REPLACE`, `UPDATE OR REPLACE`, 스키마의 `ON CONFLICT REPLACE`, Command 안의 transaction 제어와 DDL은 거부합니다.
