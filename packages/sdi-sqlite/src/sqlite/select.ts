...
export function compileSelect(plan: SelectPlan, input: Input, resources: Resources): CompiledSql {
  ...
  function pageValue(value: PageValue | undefined, input: Input, kind: 'limit' | 'offset'): number | undefined {
    ...
    if (!Number.isInteger(resolved) || (resolved as number) < minimum || (resolved as number) > 10000)
  ...
  function render(resource: string, options: SelectOptions, parent?: { alias: string; join: Join }): string {
    ...
    const column = (name: string) => {
      if (!definition.columns.includes(name)) throw new Error('UNREGISTERED_COLUMN');
    ...
    function predicateSql(predicate: Predicate): string {
      ...
      if (!['=', '<>', '<', '<=', '>', '>=', 'in', 'is-null', 'not-null'].includes(predicate.op))
        throw new Error('INVALID_PREDICATE');
    ...
    let valueSql = countRoot
      ? ''
      : options.columns
        ? `json_object(${options.columns.flatMap(name => [parameter(name), column(name)]).join(', ')})`
        : `json_object(${definition.columns.flatMap(name => [parameter(name), column(name)]).join(', ')})`;
    for (const join of options.joins ?? []) {
      ...
      const result = join.many
        ? `(SELECT coalesce(json_group_array(json(j.value)), '[]') FROM (${child}) j)`
        : `(SELECT j.value FROM (${child}) j LIMIT 1)`;
      if (plan.result !== 'count')
        valueSql = `json_set(${valueSql}, ${parameter('$."' + join.as + '"')}, json(${result}))`;
    ...
    const order = options.order?.length
      ? ' ORDER BY ' +
        options.order
          .map(
            entry =>
              `${column(entry.field)} ${entry.ascending === false ? 'DESC' : 'ASC'} NULLS ${entry.nullsFirst ? 'FIRST' : 'LAST'}`,
          )
          .join(', ')
      : '';
    ...
    const pagination = countRoot
      ? ''
      : limit !== undefined
        ? ` LIMIT ${parameter(limit)}${offset !== undefined ? ` OFFSET ${parameter(offset)}` : ''}`
        : offset !== undefined
          ? ` LIMIT -1 OFFSET ${parameter(offset)}`
          : '';
